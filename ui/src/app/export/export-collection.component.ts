...
export class ExportCollectionComponent {
  ...
  getCollectionCsv(uuid: string, entityName: string): void {
    ...
    this.collectionService
      .requestCollectionSkillsCsv(this.uuid)
  ...
  getCollectionXlsx(uuid: string, entityName: string): void {
    ...
    this.toastService.showBlockingLoader();
    this.collectionService
      .requestCollectionSkillsXlsx(this.uuid)
      .subscribe((taskStarted: ITaskResult) => {
        this.taskUuidInProgress = taskStarted.uuid;
  ...
  pollCsv(): void {
    if (this.taskUuidInProgress === undefined) {
      // fail fast
    ...
    this.collectionService
      .getCsvTaskResultsIfComplete(this.taskUuidInProgress)
      .subscribe(({ body, status }) => {
        if (status === 200) {
          ...
          FileSaver.saveAs(
            blob,
            `RSD Skills - ${this.entityName} - ${date}.csv`
          );
  ...
  pollXlsx(): void {
    ...
    this.collectionService
      .getXlsxTaskResultsIfComplete(this.taskUuidInProgress)
      .subscribe(({ body, status }) => {
        if (status === 200) {
          this.taskUuidInProgress = undefined;
          ...
          FileSaver.saveAs(
            blob,
            `RSD Skills Excel - ${this.entityName} - ${date}.xlsx`
          );
          this.toastService.hideBlockingLoader();
        }
      });
