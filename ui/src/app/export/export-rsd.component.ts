...
export class ExportRsdComponent {
  ...
  getRsdXlsx(uuid: string, entityName: string, status?: PublishStatus): void {
    ...
    const statuses = status
      ? new Set<PublishStatus>([status])
      : new Set<PublishStatus>();
    this.richSkillService
      .exportSearchXlsx(new ApiSearch({ uuids: [uuid] }), statuses)
      .subscribe(apiTask => {
        this.richSkillService
          .getResultExportedXlsxLibrary(apiTask.id.slice(1))
          .subscribe(response => {
  ...
  exportSearchCsv(
    ...
    this.richSkillService
      .exportSearchCsv(apiSearch, filterByStatuses)
      .subscribe(apiTask => {
        this.richSkillService
          .getResultExportedCsvLibrary(apiTask.id.slice(1))
          .subscribe(response => {
  ...
  exportSearchXlsx(
    ...
    this.matchingQuery = matchingQuery;
    this.richSkillService
      .exportSearchXlsx(apiSearch, filterByStatuses)
      .subscribe(apiTask => {
        this.richSkillService
          .getResultExportedXlsxLibrary(apiTask.id.slice(1))
          .subscribe(response => {
  ...
  exportLibraryCsv(): void {
    ...
    this.richSkillService
      ...
      .subscribe((apiTaskResult: ApiTaskResult) => {
        this.richSkillService
          .getResultExportedCsvLibrary(apiTaskResult.id.slice(1))
          .subscribe(response => {
            this.downloadAsCsv(response.body);
          });
      });
  ...
  exportLibraryXlsx(): void {
    ...
    this.richSkillService
      ...
      .subscribe((apiTaskResult: ApiTaskResult) => {
        this.richSkillService
          .getResultExportedXlsxLibrary(apiTaskResult.id.slice(1))
          .subscribe(response => {
            this.downloadAsXlsx(response.body);
          });
      });
