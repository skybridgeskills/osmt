...
describe('AuthGuard', () => {
  ...
  const routeMock = Object.assign({}, ActivatedRouteSnapshot.prototype, {
    data: { roles: 'ADMIN' },
  });
  const stateMock = Object.assign({}, RouterStateSnapshot.prototype, {
    url: '/cookies',
  });
  ...
  it('should return true with undefined route.data.roles', () => {
    // Arrange
    const routeWithUndefinedRoles = Object.assign(
      {},
      ActivatedRouteSnapshot.prototype,
      {
        data: { roles: undefined },
      }
    );
    // Act and Assert
    expect(authGuard.canActivate(routeWithUndefinedRoles, stateMock)).toEqual(
      true
    );
  });
});
