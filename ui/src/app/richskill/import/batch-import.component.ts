...
export const allMappingHeaderOrder = (
  alignmentCount = 3
): { field: string; label: string }[] => {
  const alignmentHeaders = [...Array(alignmentCount).keys()]
    .map(i => {
      const label = i > 0 ? ` ${i + 1}` : '';
      return [
        ...
        {
          field: 'alignmentFramework' + i,
          label: `Alignment${label} Framework`,
        },
      ];
    })
    .flat();
...
export function importSkillHeadersReverse(alignmentCount = 3): {
  [p: string]: string;
} {
  return allMappingHeaderOrder(alignmentCount).reduce(
    (acc: { [p: string]: string }, it) => {
...
export class AuditedImportSkill {
  ...
  get hasAuthors(): boolean {
    return (
      this.skill.authors?.add !== undefined &&
      this.skill.authors?.add.length > 0
    );
  }
...
export class BatchImportComponent extends QuickLinksHelper implements OnInit {
  ...
  get validCount(): number {
    return (
      this.auditedSkills?.filter(
        it => !it.isError && (this.importSimilarSkills || !it.similar)
      )?.length ?? 0
    );
  }
  ...
  isStepValid(): boolean {
    switch (this.currentStep) {
      case ImportStep.UploadFile:
        return this.parseResults !== undefined;
      case ImportStep.FieldMapping:
  ...
  duplicateMappings(): string[] | undefined {
    if (this.fieldMappings !== undefined) {
      ...
      const duplicates: string[] = Object.entries(count)
        .filter(it => it[1] > 1)
        .map(it => it[0])
  ...
  duplicateFieldNames(): string {
    const words = this.duplicateMappings()?.map(
      it => `"${importSkillHeaders[it]}"`
    );
    if (words === undefined) {
      return '';
    }
  ...
  skillsFromResults(): ApiSkillUpdate[] {
    const skillUpdates = this.parseResults?.data.map(
      (row: { [x: string]: any }) => {
        ...
        Object.keys(row).forEach(uploadedKey => {
          ...
          if (
            ...
            if (['authors'].indexOf(fieldName) !== -1) {
              ...
              ['certifications', 'employers'].indexOf(fieldName) !== -1
            ) {
              newSkill[fieldName] = new ApiReferenceListUpdate(
                value
                  .split(';')
                  .map(it => ApiNamedReference.fromString(it))
                  .filter(it => it !== undefined)
                  .map(it => it as ApiNamedReference)
              );
            } else if (['keywords'].indexOf(fieldName) !== -1) {
              newSkill[fieldName] = new ApiStringListUpdate(
                value.split(';').map(it => it.trim())
              );
            } else if (fieldName === 'occupations') {
              jobcodes.push(...value.split(';').map(it => it.trim()));
            } else if (
              (alignMatches = fieldName.match(
                /alignment(Url|Name|Framework)(\d+)/
              ))
            ) {
              const alignIndex = Number(alignMatches[2]);
              const alignField = alignMatches[1];
              ...
              if (alignField === 'Url') {
                ...
                alignmentsHolder[alignIndex].isPartOf = new ApiNamedReference({
                  name: value.trim(),
                });
              }
              ...
              newSkill[fieldName] = new ApiAlignmentListUpdate(
                value
                  .split(';')
                  .map(it => ApiAlignment.fromString(it))
                  .filter(it => it !== undefined)
                  .map(it => it as ApiAlignment)
              );
            } else {
        ...
        if (alignmentsHolder.length > 0) {
          newSkill.alignments = new ApiAlignmentListUpdate(
            Array.from(alignmentsHolder)
          );
        }
        ...
        if (
          ...
          AppConfig.settings.defaultAuthorValue.length > 0
        ) {
          newSkill['authors'] = new ApiStringListUpdate([
            AppConfig.settings.defaultAuthorValue,
          ]);
        }
    ...
    const deduped = [
      ...new Map(
        skillUpdates.map((item: { [x: string]: any }) => [
          item['skillStatement'],
          item,
        ])
      ).values(),
    ];
    const apiSkillUpdates = deduped.map(
      it => new ApiSkillUpdate(<IRichSkillUpdate>it)
    );
  ...
  private initializeMapping(): void {
    ...
    this.uploadedHeaders().forEach(it => {
      ...
      const fieldName = importSkillHeadersReverse(this.alignmentCount)[
        uploadedField
      ];
      if (fieldName) {
  ...
  private batchSimilarity(
    statements: string[]
  ): Observable<Array<ApiSkillSummary[]>> {
    ...
    const observables: Observable<Array<ApiSkillSummary[]>>[] = chunks.map(it =>
      this.richSkillService.similaritiesResults(it)
    );
  ...
  private auditRecords(): void {
    ...
    const statements: string[] = this.previewSkills
      .map(it => it.skillStatement)
      .map(it => it ?? '');
  ...
  private submitRecords(): void {
    ...
    this.importedSkills = this.auditedSkills?.filter(
      it => !it.isError && (this.importSimilarSkills || !it.similar)
    );
  ...
  get validImportSkillsCount(): boolean {
    return this.skillsToBeImported
      ? this.skillsToBeImported?.length > 0
      : false;
  }
  ...
  protected handleClickAddToWorkspace(): void {
    ...
    this.collectionService.getWorkspace().subscribe(workspace => {
      this.collectionService
        .updateSkillsWithResult(workspace.uuid, skillListUpdate, undefined)
        .subscribe(result => {
          if (result) {
  ...
  protected handleAddToExistingCollection() {
    this.router.navigate(['/collections/add-skills'], {
      state: {
        selectedSkills: this.skillsToBeImported,
        totalCount: this.skillsToBeImported?.length,
      },
    });
  }
