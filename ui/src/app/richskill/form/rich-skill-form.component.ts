...
export class RichSkillFormComponent
  ...
  implements OnInit, HasFormGroup
{
  ...
  get formValid(): boolean {
    const alignments_valid = !this.alignmentForms
  ...
  get nameErrorMessage(): string {
    return this.skillForm.get('skillName')?.hasError('notACopy')
      ? 'Name is still a copy'
      : 'Name required';
  }
  ...
  ngOnInit(): void {
    ...
    this.skillForm.controls.skillStatement.valueChanges.subscribe(
      newStatement => {
  ...
  diffUuidList(
    ...
    return removing.length > 0 || adding.length > 0
      ? new ApiStringListUpdate(adding, removing)
      : undefined;
  }
  ...
  diffNamedReferenceList(
    provided: INamedReference[],
    existing?: INamedReference[]
  ): ApiReferenceListUpdate | undefined {
    const newValues = provided?.map(v => new ApiNamedReference(v)) ?? [];
    const prevValues = existing?.map(v => new ApiNamedReference(v)) ?? [];
    const removing: ApiNamedReference[] = [...prevValues].filter(
      x => newValues.findIndex(y => x.equals(y)) === -1
    );
    const adding: ApiNamedReference[] = [...newValues].filter(
      x => prevValues.findIndex(y => x.equals(y)) === -1
    );
    return removing.length > 0 || adding.length > 0
      ? new ApiReferenceListUpdate(adding, removing)
      : undefined;
  }
  ...
  diffAlignmentList(
    provided: IAlignment[],
    existing?: IAlignment[]
  ): ApiAlignmentListUpdate | undefined {
    const newValues = provided?.map(v => new ApiAlignment(v)) ?? [];
    const prevValues = existing?.map(v => new ApiAlignment(v)) ?? [];
    const removing: ApiAlignment[] = [...prevValues].filter(
      x => newValues.findIndex(y => x.equals(y)) === -1
    );
    const adding: ApiAlignment[] = [...newValues].filter(
      x => prevValues.findIndex(y => x.equals(y)) === -1
    );
    return removing.length > 0 || adding.length > 0
      ? new ApiAlignmentListUpdate(adding, removing)
      : undefined;
  }
  ...
  updateObject(): ApiSkillUpdate {
    ...
    if (
      inputName &&
      (this.isDuplicating || this.existingSkill?.skillName !== inputName)
    ) {
    ...
    if (
      inputStatement &&
      (this.isDuplicating ||
        this.existingSkill?.skillStatement !== inputStatement)
    ) {
    ...
    const occupationsDiff = this.diffStringList(
      ...
      this.existingSkill?.occupations?.map((it: IJobCode) =>
        this.stringFromJobCode(it)
      )
    );
    ...
    const alignments = this.alignmentForms
      .map(group => {
        ...
        return new ApiAlignment({
          ...
          isPartOf: frameworkName
            ? new ApiNamedReference({ name: frameworkName })
            : undefined,
        });
      })
  ...
  onSubmit(): void {
    ...
    if (this.skillUuid && !this.isDuplicating) {
      this.skillSaved = this.richSkillService.updateSkill(
        this.skillUuid,
        updateObject
      );
    } else {
  ...
  setSkill(skill: ApiSkill): void {
    ...
    if (!this.isDuplicating) {
      this.titleService.setTitle(
        `${skill.skillName} | ${this.pageTitle()} | ${this.whitelabel.toolName}`
      );
    }

    const firstAlignment: IAlignment | undefined = skill.alignments?.find(
      it => true
    );
  ...
  checkForStatementSimilarity(statement: string): void {
    ...
    this.richSkillService.similarityCheck(statement).subscribe(results => {
      this.similarSkills = this.isDuplicating
        ? results
        : results?.filter(s => this.existingSkill?.uuid !== s.uuid);
