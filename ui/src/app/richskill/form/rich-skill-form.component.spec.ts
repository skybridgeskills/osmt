...
describe('RichSkillFormComponent', () => {
  ...
  it('pageTitle should return title', () => {
    ...
    component.existingSkill = new ApiSkill(
      createMockSkill(date, date, PublishStatus.Draft)
    );
    // Act
  ...
  it('nameErrorMessage should return error ', () => {
    // Arrange
    component.skillForm.get('skillName')?.setValue('unique name (Copy 2024-01-01 10:00)');
    // Act
  ...
  it('skillStatementErrorMessage should return error ', () => {
    // Assert
    expect(component.skillStatementErrorMessage).toEqual(
  ...
  it('diffUuidList should be correct', () => {
    ...
    const updates1 = component.diffUuidList(
      ['aardvark', 'giraffe', 'zebra'],
      collections
    );
    // Assert
    expect(updates1).toEqual(
      new ApiStringListUpdate(['giraffe'], ['hippo', 'fox'])
    );

    // Act - adding 0, removing 2
    const updates2 = component.diffUuidList(['aardvark', 'zebra'], collections);
    // Assert
    expect(updates2).toEqual(new ApiStringListUpdate([], ['hippo', 'fox']));

    // Act - adding 1, removing 0
    const updates3 = component.diffUuidList(
      ['aardvark', 'giraffe', 'hippo', 'zebra', 'fox'],
      collections
    );
    // Assert
    expect(updates3).toEqual(new ApiStringListUpdate(['giraffe'], []));

    // Act
    const updates4 = component.diffUuidList(
      ['fox', 'aardvark', 'hippo', 'zebra'],
      collections
    );
    // Assert
  ...
  it('diffStringList should be correct', () => {
    // Arrange
    const keywords = ['zebra', 'hippo', 'fox', 'aardvark'];

    // Act - adding 1, removing 2
    const updates1 = component.diffStringList(
      ['aardvark', 'giraffe', 'zebra'],
      keywords
    );
    // Assert
    ...
    // Act - adding 1, removing 0
    const updates3 = component.diffStringList(
      ['aardvark', 'giraffe', 'hippo', 'zebra', 'fox'],
      keywords
    );
    // Assert
    ...
    // Act
    const updates4 = component.diffStringList(
      ['fox', 'aardvark', 'hippo', 'zebra'],
      keywords
    );
    // Assert
  ...
  it('diffReferenceList should be correct', () => {
    ...
    // Assert
    expect(updates1).toEqual(
      new ApiReferenceListUpdate(
        [new ApiNamedReference(references[4])],
        [
          new ApiNamedReference(references[1]),
          new ApiNamedReference(references[2]),
        ]
      )
    );
    ...
    expect(updates3).toEqual(
      new ApiReferenceListUpdate([new ApiNamedReference(references[4])], [])
    );
  ...
  it('updateObject should return updates', () => {
    ...
    const {
      ...
      occupations: [
        new ApiJobCode({ code: 'occp1' }),
        new ApiJobCode({ code: 'occp2' }),
      ],
      standards: [
        ApiAlignment.fromString('stnd1'),
        ApiAlignment.fromString('stnd2'),
      ].filter(v => !!v),
    };
    ...
    expect((update.keywords as IStringListUpdate).add).toEqual(
      keywords as string[]
    );
    expect((update.standards as IReferenceListUpdate).add).toEqual(
      standards as INamedReference[]
    );
    expect((update.collections as IStringListUpdate).add).toEqual(collections);
    expect((update.certifications as IReferenceListUpdate).add).toEqual(
      certifications as INamedReference[]
    );
    expect((update.occupations as IStringListUpdate).add).toEqual(
      occupations.map(o => component.stringFromJobCode(o))
    );
    expect((update.employers as IReferenceListUpdate).add).toEqual(
      employers as INamedReference[]
    );
  });
  ...
  it('setSkill should be correct', () => {
    ...
    expect(titleService.getTitle()).toEqual(
      `${skill.skillName} | Edit Rich Skill Descriptor | OSMT`
    );
  });
