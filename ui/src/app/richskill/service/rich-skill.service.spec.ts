...
describe('RichSkillService', () => {
  ...
  it('getSkills filter should return', () => {
    ...
    // Act
    // noinspection LocalVariableNamingConventionJS
    const result$ = testService.getSkillsFiltered(
      testData.skills.length,
      0,
      new ApiSearch({ filtered: {} }),
      statuses,
      ApiSortOrder.SkillAsc
    );

    // Assert
    result$.subscribe((data: PaginatedSkills) => {
  ...
  it('getSkillByUUID should return', () => {
    ...
    const testData: ApiSkill = new ApiSkill(
    ...
    // Assert
    result$.subscribe((data: ApiSkill) => {
      expect(data).toEqual(testData);
      expect(RouterData.commands).toEqual([]); // No errors
      expect(AuthServiceData.isDown).toEqual(false);
    });
    ...
    try {
      ...
      if (e instanceof Error) {
        expect(e.message).toEqual(
          'No uuid provided for single skill csv export'
        );
      }
    }
  });
  ...
  it('getSkillJsonByUUID should return', () => {
    ...
    const path = getBaseApi() + '/skills/' + uuid;
    const testData =
      // Doesn't matter what the string has in it.
      `{
          "foo": "bar"
       }`;
  ...
  it('getSkillJsonByUuid should fail', () => {
    ...
    try {
      ...
      if (e instanceof Error) {
        expect(e.message).toEqual(
          'No uuid provided for single skill json export'
        );
      }
    }
  });
  ...
  it('createSkill should return', fakeAsync(() => {
    ...
    const now = new Date();
    const skillResult: ApiSkill = new ApiSkill(
      createMockSkill(now, now, PublishStatus.Published)
    );
    ...
    const req1 = httpTestingController.expectOne(
      AppConfig.settings.baseApiUrl + path1
    );
    ...
    const req2 = httpTestingController.expectOne(
      AppConfig.settings.baseApiUrl + path2
    );
  ...
  it('updateSkill should return', () => {
    ...
    const now = new Date();
    const testData: ApiSkill = new ApiSkill(
      createMockSkill(now, now, PublishStatus.Draft)
    );
  ...
  it('searchSkills should return', () => {
    ...
    const size = 5;
    const from = 1;
    const filter = new Set<PublishStatus>([
    ...
    // Act
    // noinspection LocalVariableNamingConventionJS
    const result$ = testService.searchSkills(
      apiSearch,
      size,
      from,
      filter,
  ...
  it('libraryExport should return', fakeAsync(() => {
    ...
    const req = httpTestingController.expectOne(
      AppConfig.settings.baseApiUrl + getBaseApi() + '/export/library/csv'
    );
    expect(req.request.method).toEqual('GET');
    expect(req.request.headers.get('Accept')).toEqual('application/json');
    req.flush(result$);
  ...
  it('publishSkillsWithResult should return', fakeAsync(() => {
    ...
    /* Setup for request 1 */
    const req1 = httpTestingController.expectOne(
      AppConfig.settings.baseApiUrl + path1 + '?newStatus=published'
    );
    expect(req1.request.method).toEqual('POST');
    req1.flush(taskResult);
  ...
  it('auditLog should return', () => {
    ...
    // Assert
    result$.subscribe((data: ApiAuditLog[]) => {
      expect(data).toEqual(testData);
      expect(RouterData.commands).toEqual([]); // No errors
      expect(AuthServiceData.isDown).toEqual(false);
    });
  ...
  it('similarityCheck should return', () => {
    ...
    // Assert
    result$.subscribe((data: ApiSkillSummary[]) => {
      expect(data).toEqual(testData);
      expect(RouterData.commands).toEqual([]); // No errors
      expect(AuthServiceData.isDown).toEqual(false);
    });
  ...
  it('similaritiesCheck should return', () => {
    ...
    // Assert
    result$.subscribe((data: Array<ApiSkillSummary[]>) => {
      expect(data).toEqual(testData);
      expect(RouterData.commands).toEqual([]); // No errors
      expect(AuthServiceData.isDown).toEqual(false);
    });
