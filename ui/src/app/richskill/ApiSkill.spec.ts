...
describe('ApiSkill', () => {
  it('ApiUuidReference should be created', () => {
    ...
    expect(deepEqualSkipOuterType(apiUuidReference, iUuidReference)).toBeTruthy(
      mismatched(apiUuidReference, iUuidReference)
    );
  });
  ...
  it('ApiNamedReference should be created', () => {
    ...
    expect(
      deepEqualSkipOuterType(apiNamedReference, iNamedReference)
    ).toBeTruthy(mismatched(apiNamedReference, iNamedReference));
  });
  ...
  it('ApiNamedReference should be equal', () => {
    ...
    const e = new ApiNamedReference(
      createMockNamedReference(undefined, undefined)
    );
  ...
  it('ApiNamedReference should be id from text', () => {
    ...
    expect(ApiNamedReference.fromString('a://b')).toEqual(
      new ApiNamedReference({ id: 'a://b' })
    );
    expect(ApiNamedReference.fromString('abc')).toEqual(
      new ApiNamedReference({ name: 'abc' })
    );
  });
  ...
  it('ApiAuditLog should be created', () => {
    ...
    expect(apiAuditLog.creationDate?.toISOString()).toEqual(
      iAuditLog.creationDate
    );
    expect(apiAuditLog.operationType).toEqual(iAuditLog.operationType);
    expect(apiAuditLog.user).toEqual(iAuditLog.user);
    expect(
      _.isEqual(apiAuditLog.changedFields, iAuditLog.changedFields)
    ).toBeTruthy();
  });
  ...
  it('ApiAuditLog should match status', () => {
    ...
    const i3 = new ApiAuditLog(
      createMockAuditLog(AuditOperationType.PublishStatusChange)
    );
  ...
  it('ApiSkill should be created', () => {
    ...
    expect(apiSkill.alignments).toEqual(
      iSkill.alignments.map(v => new ApiAlignment(v))
    );
    expect(apiSkill.standards).toEqual(
      iSkill.standards.map(v => new ApiAlignment(v))
    );
    expect(apiSkill.certifications).toEqual(
      iSkill.certifications.map(v => new ApiNamedReference(v))
    );
    expect(apiSkill.occupations).toEqual(
      iSkill.occupations.map(v => new ApiJobCode(v))
    );
    expect(apiSkill.employers).toEqual(
      iSkill.employers.map(v => new ApiNamedReference(v))
    );
