...
export class ApiAlignment implements IAlignment {
  ...
  constructor(reference: IAlignment) {
    ...
    this.isPartOf = reference.isPartOf
      ? new ApiNamedReference(reference.isPartOf)
      : undefined;
  }
...
export class ApiSkill {
  ...
  constructor(iRichSkill: ISkill) {
    ...
    this.certifications =
      iRichSkill.certifications?.map(it => new ApiNamedReference(it)) ?? null;
    this.alignments =
      iRichSkill.alignments?.map(it => new ApiAlignment(it)) ?? null;
    this.standards =
      iRichSkill.standards?.map(it => new ApiAlignment(it)) ?? null;
  ...
  get sortedAlignments(): IAlignment[] {
    return [...this.alignments].sort((a, b) => {
      const fwk = a.isPartOf
        ? a.isPartOf?.name?.localeCompare(b.isPartOf?.name ?? '')
        : 1;
      const name = a.skillName
