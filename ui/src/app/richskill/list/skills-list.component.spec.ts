...
describe('SkillsListComponent', () => {
  ...
  it('addToCollectionVisible should be correct', () => {
    // No skills
  ...
  it('selectedUuids should be correct', () => {
    ...
    expect(component.selectedUuids(undefined)).toEqual([
      component.selectedSkills[0].uuid,
    ]);

    const skill = createMockSkillSummary();
    expect(component.selectedUuids()).toEqual([skill.uuid]);
  });
  ...
  it('getApiSearch should be correct', () => {
    ...
    expect(component.getApiSearch(skill)).toEqual(new ApiSearch({ uuids }));
  ...
  it('submitStatusChange should be correct', () => {
    ...
    const skill = createMockSkillSummary('id2', PublishStatus.Draft);
    expect(
      component.submitStatusChange(PublishStatus.Published, 'published', skill)
    ).toEqual(false);
  });
