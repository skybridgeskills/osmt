...
export class SkillsListComponent extends QuickLinksHelper {
  ...
  publishVisible(skill?: ApiSkillSummary): boolean {
    ...
    if (skill !== undefined) {
      return (
        ...
        this.authService.isEnabledByRoles(ButtonAction.SkillPublish)
      );
    } else if ((this.selectedSkills?.length ?? 0) === 0) {
      return false;
    } else {
      const unpublishedSkill = this.selectedSkills?.find(
        s => s.publishDate === undefined
      );
      return (
        unpublishedSkill !== undefined &&
        this.authService.isEnabledByRoles(ButtonAction.SkillPublish)
      );
    }
  }
  archiveVisible(skill?: ApiSkillSummary): boolean {
    ...
    if (skill !== undefined) {
      return (
        !checkArchived(skill) &&
        this.authService.isEnabledByRoles(ButtonAction.SkillUpdate)
      );
    } else if ((this.selectedSkills?.length ?? 0) === 0) {
      return false;
    } else {
      const unarchivedSkills = this.selectedSkills?.find(
        s => !checkArchived(s)
      );
      return (
        unarchivedSkills !== undefined &&
        this.authService.isEnabledByRoles(ButtonAction.SkillUpdate)
      );
    }
  }
  ...
  addToCollectionVisible(skill?: ApiSkillSummary): boolean {
    ...
    return (
      this.addToVisible() &&
      this.authService.isEnabledByRoles(ButtonAction.CollectionSkillsUpdate)
    );
  }
  ...
  rowActions(): TableActionDefinition[] {
    const actions = [
      new TableActionDefinition({
        label: 'Archive RSD',
        callback: (action: TableActionDefinition, skill?: ApiSkillSummary) =>
          this.handleClickArchive(action, skill),
        visible: (skill?: ApiSkillSummary) => this.archiveVisible(skill),
      }),
      new TableActionDefinition({
        label: 'Unarchive RSD',
        callback: (action: TableActionDefinition, skill?: ApiSkillSummary) =>
          this.handleClickUnarchive(action, skill),
        visible: (skill?: ApiSkillSummary) => this.unarchiveVisible(skill),
      }),
      new TableActionDefinition({
        label: 'Publish RSD',
        callback: (action: TableActionDefinition, skill?: ApiSkillSummary) =>
          this.handleClickPublish(action, skill),
    ...
    if (this.showAddToCollection) {
      actions.push(
        new TableActionDefinition({
          ...
          callback: (action: TableActionDefinition, skill?: ApiSkillSummary) =>
            this.handleClickAddCollection(action, skill),
          visible: (skill?: ApiSkillSummary) =>
            this.addToCollectionVisible(skill),
        })
      );
    } else {
      actions.push(
        new TableActionDefinition({
          ...
          callback: (action: TableActionDefinition, skill?: ApiSkillSummary) =>
            this.handleClickRemoveCollection(action, skill),
          visible: (skill?: ApiSkillSummary) =>
            this.addToCollectionVisible(skill),
        })
      );
  ...
  tableActions(): TableActionDefinition[] {
    const actions = [
      new TableActionDefinition({
        ...
        offset: true,
        callback: (action: TableActionDefinition, skill?: ApiSkillSummary) =>
          this.handleClickBackToTop(action, skill),
      ...
      actions.push(
        new TableActionDefinition({
          ...
          callback: (action: TableActionDefinition, skill?: ApiSkillSummary) =>
            this.handleClickRemoveCollection(action, skill),
          visible: (skill?: ApiSkillSummary) =>
            this.addToCollectionVisible(skill) || this.addToWorkspaceVisible(),
        })
      );
    }
  ...
  protected handleClickAddToWorkspace(): void {
    const skillListUpdate = this.selectAllChecked
      ? new ApiSkillListUpdate({
          add: new ApiSearch({
            query: this.matchingQuery?.join(''),
            filtered: {},
          }),
        })
      : new ApiSkillListUpdate({
          add: new ApiSearch({
            uuids: this.getSelectedSkills()?.map(i => i.uuid),
          }),
        });
  ...
  protected handleClickAddCollection(
    action: TableActionDefinition,
    skill?: ApiSkillSummary
  ): boolean {
  ...
  private handleClickRemoveCollection(
    action: TableActionDefinition,
    skill?: ApiSkillSummary
  ): boolean {
  ...
  protected onlyDraftsSelected(skill?: ApiSkillSummary): boolean {
    return (
      skill?.status === PublishStatus.Draft ||
      this.selectedSkills?.find(it => it.status !== PublishStatus.Draft) ===
        undefined
    );
  }
  ...
  private handleClickPublish(
    action: TableActionDefinition,
    skill?: ApiSkillSummary
  ): boolean {
    const plural =
      (this.selectedUuids(skill)?.length ?? 0) > 1 ? 'these RSDs' : 'this RSD';
    if (
      confirm(
        `Are you sure you want to publish ${plural}?\nOnce published, an RSD can't be unpublished.`
      )
    ) {
  ...
  submitStatusChange(
    ...
    this.toastService.showBlockingLoader();
    this.skillsSaved = this.richSkillService.publishSkillsWithResult(
      apiSearch,
      newStatus,
    ...
    this.skillsSaved.subscribe(result => {
      if (result !== undefined) {
        const partial =
          result.modifiedCount !== result.totalCount
