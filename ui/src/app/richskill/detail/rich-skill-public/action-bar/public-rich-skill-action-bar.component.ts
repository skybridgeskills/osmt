...
export class PublicRichSkillActionBarComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.skillJsonObservable = this.richSkillService.getSkillJsonByUuid(
      this.skillUuid
    );
    this.skillJsonObservable.subscribe(
      (json: string) => (this.jsonClipboard = json)
    );
  }
      ...
      menu: [
        {
          ...
          visible: () => true,
          callback: () =>
            this.exporter.getRsdCsv(this.skillUuid, this.skillName),
        },
        {
          label: 'Download as Excel Workbook',
          visible: () => true,
          callback: () =>
            this.exporter.getRsdXlsx(
              this.skillUuid,
              this.skillName,
              this.skillStatus
            ),
