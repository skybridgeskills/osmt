...
@Component({
  ...
  template: `
    <div>
      <div
        *ngIf="!!majorCodes && !isCollapsed"
        class="t-margin-small t-margin-top"
      >
      ...
      <div class="t-margin-small t-margin-top">
        <h4 *ngIf="!!detailedCodes" class="t-type-bodyBoldCaps">
        ...
        <div *ngFor="let code of detailedCodes" class="l-flex">
          ...
          <p>
            <a
              href="https://www.bls.gov/oes/current/oes{{
                code.code.replace('-', '')
              }}.htm"
              target="_blank"
              >{{ code.targetNodeName }}</a
            >
          </p>
        </div>
      </div>
      ...
      <div class="t-margin-small t-margin-top">
        ...
        <div *ngFor="let code of onetCodes" class="l-flex">
          ...
          <p>
            <a
              href="https://onetonline.org/link/summary/{{ code.code }}"
              target="_blank"
              >{{ code.targetNodeName }}</a
            >
          </p>
...
export class OccupationsCardSectionComponent implements OnInit {
  ...
  distinctJobcodes(input: Array<IJobCode>): Array<IJobCode> {
    return input
      ...
      .filter(
        (item, idx, arr) => arr.findIndex(it => it.code === item.code) === idx
      )
  ...
  major(): Set<IJobCode> {
    return new Set(
      this.distinctJobcodes(
        this.codes?.flatMap(
          code => code.parents?.filter(p => p.level === 'Major') ?? []
        )
      )
    );
  }
  ...
  minor(): Set<IJobCode> {
    return new Set(
      this.distinctJobcodes(
        this.codes?.flatMap(
          code => code.parents?.filter(p => p.level === 'Minor') ?? []
        )
      )
    );
  }
  ...
  broad(): Set<IJobCode> {
    return new Set(
      this.distinctJobcodes(
        this.codes?.flatMap(
          code => code.parents?.filter(p => p.level === 'Broad') ?? []
        )
      )
    );
  }
  detailed(): Set<IJobCode> {
    return new Set(
      this.distinctJobcodes(
        this.codes?.flatMap(
          code => code.parents?.filter(p => p.level === 'Detailed') ?? []
        )
      )
    );
  }
  ...
  onet(): Set<IJobCode> {
    return new Set(
      this.distinctJobcodes(
        this.codes?.filter(code => code.frameworkName === 'o*net')
      )
    );
  }
}
