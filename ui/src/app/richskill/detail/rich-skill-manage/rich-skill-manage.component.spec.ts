...
export function createComponent(
  T: Type<RichSkillManageComponent>
): Promise<void> {
...
describe('RichSkillManageComponent', () => {
  beforeEach(waitForAsync(() => {
    TestBed.configureTestingModule({
      declarations: [RichSkillManageComponent],
      imports: [
  ...
  it('getCardFormat should return', () => {
    ...
    const expected: IDetailCardSectionData[] = [
      ...
      {
        label: 'Alignments',
        bodyString: skill.alignments
          .map(
            it =>
              `<p class="t-type-body">${it.isPartOf?.name + ': '}<a class="t-link" target="_blank" href="${it.id}">${it.skillName || it.id}</a></p>`
          )
          .join('; '),
        showIfEmpty: true,
      },
      {
        ...
        bodyString: skill.collections
          .map(
            it =>
              `<div><a class="t-link" href="/collections/${it.uuid}/manage">${it.name}</a></div>`
          )
          .join('<br>'),
        showIfEmpty: false,
      },
