  ...
  getCardFormat(): IDetailCardSectionData[] {
    return [
      ...
      },
      {
        label: 'Standards',
        bodyString:
          this.richSkill?.standards
            ?.map(standard => standard.skillName)
            ?.join('; ') ?? '',
        showIfEmpty: true,
      },
      {
      ...
      },
      {
        label: 'Alignments',
        bodyString:
          this.richSkill?.sortedAlignments
            ?.map(alignment => {
              const framework = alignment.isPartOf
                ? `${alignment.isPartOf.name}: `
                : '';
              return alignment.id
