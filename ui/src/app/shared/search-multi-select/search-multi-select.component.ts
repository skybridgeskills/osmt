  ...
  implements OnInit
{
  ...
  selectResult(result: ApiJobCode | ApiNamedReference): void {
    ...
    if (!isResultSelected) {
      ...
      const newValues = this.control?.value.filter(
        (r: ApiJobCode | ApiNamedReference) => !this.areResultsEqual(r, result)
      );
  ...
  private getKeywords(text: string): void {
    ...
    this.keywordType
      ...
      : this.searchService.searchJobcodes(text).subscribe(searchResults => {
          this.results = searchResults.filter(
            r => !!r && !!r.code && !!r.targetNodeName
          );
          this.currentlyLoading = false;
        });
  }
  ...
  isResultSelected(result: ApiJobCode | ApiNamedReference): boolean {
    return this.control?.value.some((i: ApiJobCode | ApiNamedReference) =>
      this.areResultsEqual(i, result)
    );
  }
