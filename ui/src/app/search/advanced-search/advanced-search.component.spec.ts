...
export function createComponent(
  T: Type<AdvancedSearchComponent>
): Promise<void> {
...
function setupForm(isBlank: boolean): object {
  ...
  const fields = isBlank
    ...
    : {
        ...
        standards: ['standard1', 'standard2', 'standard3'].map(
          v => new ApiNamedReference({ name: v })
        ),
        certifications: [
          'certification1',
          'certification2',
          'certification3',
        ].map(v => new ApiNamedReference({ name: v })),
        occupations: ['occupation1', 'occupation2', 'occupation3'].map(
          v => new ApiJobCode({ code: v })
        ),
        employers: ['employer1', 'employer2', 'employer3'].map(
          v => new ApiNamedReference({ name: v })
        ),
        alignments: ['alignment1', 'alignment2'].map(
          v => new ApiNamedReference({ name: v })
        ),
