...
export class AdvancedSearchComponent extends Whitelabelled implements OnInit {
  ...
  private collectFieldData(isSearchingCollections = false): ApiAdvancedSearch {
    ...
    const keywords =
      form.keywords && form.keywords.length > 0
        ? form.keywords.map((v: INamedReference) => v)
        : undefined;
    // Necessary because of type mismatch between ApiSkill(IAlignment[]) & ApiAdvancedSearch(INamedReference[])
    const standards =
      form.standards && form.standards.length > 0
        ? form.standards.map(
            (v: IAlignment) => new ApiNamedReference({ name: v.skillName })
          )
        : undefined;
    const certifications =
      form.certifications && form.certifications.length > 0
        ? form.certifications.map((v: INamedReference) => v)
        : undefined;
    const occupations =
      form.occupations && form.occupations.length > 0
        ? form.occupations.map((v: IJobCode) => v.code)
        : undefined;
    const employers =
      form.employers && form.employers.length > 0
        ? form.employers.map((v: INamedReference) => v)
        : undefined;
    // Necessary because of type mismatch between ApiSkill(IAlignment[]) & ApiAdvancedSearch(INamedReference[])
    const alignments =
      form.alignments && form.alignments.length > 0
        ? form.alignments.map(
            (v: IAlignment) => new ApiNamedReference({ name: v.skillName })
          )
        : undefined;
