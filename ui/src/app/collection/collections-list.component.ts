...
export class CollectionsListComponent extends Whitelabelled {
  ...
  archiveVisible(collection?: ApiCollectionSummary): boolean {
    if (collection !== undefined) {
      return (
        !checkArchived(collection) &&
        this.authService.isEnabledByRoles(ButtonAction.CollectionUpdate)
      );
    } else if ((this.selectedCollections?.length ?? 0) === 0) {
      return false;
    } else {
      const unarchCollection = this.selectedCollections?.find(
        s => !checkArchived(s)
      );
      return (
        unarchCollection !== undefined &&
        this.authService.isEnabledByRoles(ButtonAction.CollectionUpdate)
      );
    }
  }
  ...
  private handleClickPublish(
    ...
    const plural =
      (this.selectedUuids(skill)?.length ?? 0) > 1
        ? 'these collections'
        : 'this collection';
    if (
      confirm(
  ...
  getSelectedSkills(
    skill?: ApiCollectionSummary
  ): ApiCollectionSummary[] | undefined {
  ...
  submitStatusChange(
    ...
    this.collectionsSaved = this.collectionService.publishCollectionsWithResult(
      apiSearch,
      newStatus,
