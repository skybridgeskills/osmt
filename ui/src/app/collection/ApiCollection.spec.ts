...
describe('ApiCollection', () => {
  it('ApiCollection should be created', () => {
    ...
    const iCollection: ICollection = createMockCollection(
      date,
      date,
      date,
      date,
    ...
    expect(deepEqualSkipOuterType(apiCollection, iCollection)).toBeTruthy(
      mismatched(apiCollection, iCollection)
    );
  });
  ...
  it('ApiCollectionUpdate should be created', () => {
    // Arrange
    const date = new Date('2020-06-25T14:58:46.313Z');
    const iCollectionUpdate: ICollectionUpdate = createMockCollectionUpdate(
      date,
      date,
      date,
      date,
    ...
    expect(
      deepEqualSkipOuterType(apiCollectionUpdate, iCollectionUpdate)
    ).toBeTruthy(mismatched(apiCollectionUpdate, iCollectionUpdate));
  });
});
