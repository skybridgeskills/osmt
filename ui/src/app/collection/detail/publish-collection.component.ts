...
export class PublishCollectionComponent
  ...
  implements OnInit
{
  ...
  nextState(): void {
    ...
    if (this.activeState === PubColState.checkingArchived) {
      this.checkForStatus(
        new Set([PublishStatus.Archived, PublishStatus.Deleted])
      );
  ...
  handleClickConfirmPublish(): boolean {
    ...
    this.submitSkillStatusChange(
      ...
      new Set([PublishStatus.Draft])
    );
    return false;
  }

  submitSkillStatusChange(
    newStatus: PublishStatus,
    filterStatuses: Set<PublishStatus>
  ): void {
