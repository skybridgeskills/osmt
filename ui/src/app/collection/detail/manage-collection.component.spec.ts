...
describe('ManageCollectionComponent', () => {
  beforeEach(waitForAsync(() => {
    TestBed.configureTestingModule({
      ...
      imports: [
        RouterTestingModule.withRoutes([
          {
            path: 'collections/uuid1/add-skills',
            component: ManageCollectionComponent,
          },
          {
            path: 'collections/UUID1/edit',
            component: ManageCollectionComponent,
          },
          {
    ...
    component.titleElement = new ElementRef(
      document.getElementById('titleHeading')
    );
  }));
  ...
  it('loadNextPage should be correct', () => {
    ...
    // Assert
    expect((component.results as PaginatedSkills).totalCount).toEqual(
      totalCount
    );
  });
  ...
  it('handleConfirmDeleteCollection should call', () => {
    ...
    const spyDeleteCollectionWithResult = spyOn(
  ...
  it('publishAction should be correct', () => {
    ...
    const spySubmitCollectionStatusChange = spyOn(
      component,
      'submitCollectionStatusChange'
    ).and.stub();
    let readyUuid;
    spyOn(collectionService, 'collectionReadyToPublish').and.callFake(uuid => {
      readyUuid = uuid;
  ...
  it('removeFromCollection should be correct', () => {
    // Arrange for allo
    const spySubmitSkillRemoval = spyOn(
      component,
      'submitSkillRemoval'
    ).and.returnValue(); // Test just this method
  ...
  it('remove skills should have api search with query', () => {
    ...
    component.removeFromCollection();
    const date = new Date();
    component.collection = createMockCollection(
      date,
      date,
      date,
      date,
  ...
  it('submitSkillRemoval should be correct', () => {
    // Arrange
    const spyReloadCollection = spyOn(
      component,
      'reloadCollection'
    ).and.returnValue();
  ...
  it('Download should have submenu', () => {
    const actionDownload = component
      .actionDefinitions()
  ...
  it('Menu action download should call callback correctly', () => {
    const actionDownload = component
      .actionDefinitions()
      .find(i => i.label === 'Download');
    const downloadCsv = actionDownload?.menu
      ? actionDownload.menu[0]
      : undefined;
    const downloadXlsx = actionDownload?.menu
      ? actionDownload.menu[1]
      : undefined;
