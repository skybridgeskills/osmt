...
export class AddSkillsCollectionComponent
  ...
  implements OnInit
{
  ...
  constructor(
    ...
    this.state = this.router.getCurrentNavigation()?.extras
      .state as ExtrasSelectedSkillsState;
  ...
  rowActions(): TableActionDefinition[] {
    return [
      new TableActionDefinition({
        ...
        ) => this.handleSelectCollection(action, collection),
      }),
    ];
  }

  private handleSelectCollection(
    action: TableActionDefinition,
    collection?: ApiCollectionSummary
  ): boolean {
    ...
    const update = new ApiSkillListUpdate({
      add:
        this.state?.search !== undefined
          ? this.state?.search
          : new ApiSearch({
              uuids: this.state?.selectedSkills?.map(it => it.uuid),
            }),
    });

    this.toastService.showBlockingLoader();
    this.collectionService
      .updateSkillsWithResult(collection.uuid, update)
      .subscribe(result => {
        if (result) {
