...
export class CollectionService extends AbstractService {
  ...
  requestCollectionSkillsCsv(uuid: string): Observable<ITaskResult> {
    ...
    return this.get<ITaskResult>({
      ...
      .pipe(
        map(
          ({ body }) => new ApiTaskResult(this.safeUnwrapBody(body, errorMsg))
        )
      );
  }
  ...
  createCollection(updateObject: ICollectionUpdate): Observable<ApiCollection> {
    ...
    return this.post<ApiCollection[]>({
      ...
      .pipe(
        map(
          ({ body }) =>
            this.safeUnwrapBody(body, errorMsg).map(
              s => new ApiCollection(s)
            )[0]
        )
      );
  }
    ...
    return this.post<ICollectionSummary[]>({
      ...
      .pipe(
        map(({ body, headers }) => {
          ...
          const collections =
            body?.map(it => new ApiCollectionSummary(it)) || [];
          return new PaginatedCollections(
            collections,
            !isNaN(totalCount) ? totalCount : collections.length
          );
        })
      );
  }
  ...
  updateSkills(
    ...
    const params = this.buildTableParams(
      undefined,
      undefined,
      filterByStatuses,
      undefined
    ...
    return this.post<ITaskResult>({
      ...
      .pipe(
        map(
          ({ body }) =>
            new ApiTaskResult(this.safeUnwrapBody(body, 'unwrap failure'))
        )
      );
  }
  ...
  updateSkillsWithResult(
    ...
    return this.pollForTaskResult<ApiBatchResult>(
      this.updateSkills(collectionUuid, skillListUpdate, filterByStatus),
      pollIntervalMs
    );
  }
  ...
  deleteCollection(uuid: string): Observable<ApiTaskResult> {
    return this.httpClient
      .delete<ITaskResult>(this.buildUrl('collections/' + uuid + '/remove'), {
        headers: this.wrapHeaders(
          new HttpHeaders({
            Accept: 'application/json',
          })
      ...
      .pipe(
        map(
          body => new ApiTaskResult(this.safeUnwrapBody(body, 'unwrap failure'))
        )
      );
  }
  ...
  collectionReadyToPublish(collectionUuid: string): Observable<boolean> {
    return new Observable(observer => {
      ...
      this.getCollectionSkills(collectionUuid, 1, 0, filters).subscribe(
        results => {
  ...
  auditLog(uuid?: string): Observable<ApiAuditLog[]> {
  ...
  private collectionFromApiResponse(
    body: any,
    ...
    Object.keys(body.skillKeywords).forEach(k => {
      const kws = body.skillKeywords[k]
        ? (body.skillKeywords[k] as IKeywordCount[])
        : [];
