...
export class CollectionSkillSearchComponent
  ...
  implements OnInit
{
  ...
  loadNextPage(): void {
    ...
    this.resultsLoaded = this.richSkillService.searchSkills(
      apiSearch,
      this.size,
      this.from,
  ...
  protected handleClickAddCollection(
    ...
    const selectedCount = this.multiplePagesSelected
      ? this.totalCount
      : (this.selectedSkills?.length ?? 1);
