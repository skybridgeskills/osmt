...
export abstract class AbstractService {
  ...
  post<T>({
    ...
    params,
    body,
  }: ApiGetParams): Observable<HttpResponse<T>> {
    const observable = this.httpClient
  ...
  pollForTaskResult<T>(
    obs: Observable<ApiTaskResult>,
    pollIntervalMs = 1000
  ): Observable<T> {
    return new Observable(observer => {
      obs.subscribe(task => {
        this.observableForTaskResult<T>(task, pollIntervalMs).subscribe(
          result => {
            if (result !== undefined) {
  ...
  bulkStatusChange(
    ...
    if (filterByStatuses !== undefined) {
      params.filterByStatus = Array.from(filterByStatuses).map(s =>
        s.toString()
      );
    }
