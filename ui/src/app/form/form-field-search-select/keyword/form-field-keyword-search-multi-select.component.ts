...
@Component({
  selector: 'app-form-field-keyword-search-multi-select',
  templateUrl: '../abstract-form-field-search-multi-select.component.html',
})
export class FormFieldKeywordSearchMultiSelect extends AbstractFormFieldSearchMultiSelect<
  IAlignment | INamedReference | string
> {
  ...
  isSearchResultType(
    result: any
  ): result is IAlignment | INamedReference | string {
    return (
      isAlignment(result) || isNamedReference(result) || isStringKeyword(result)
    );
  }
  ...
  protected callSearchService(
    text: string
  ): Observable<(IAlignment | INamedReference | string)[]> {
    if (!this.keywordType) {
    ...
    let transformFn = (
      r: INamedReference
    ): IAlignment | INamedReference | string | undefined => r;
    ...
    return this.searchService
      .searchKeywords(this.keywordType, text)
      .pipe(
        map((results): (IAlignment | INamedReference | string)[] =>
          results.flatMap(r => transformFn(r) ?? [])
        )
      );
  }
}
