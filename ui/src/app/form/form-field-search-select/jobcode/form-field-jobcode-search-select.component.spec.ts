...
@Component({
  template:
    '<app-form-field-jobcode-search-select></app-form-field-jobcode-search-select>',
})
export abstract class TestHostComponent extends FormFieldJobCodeSearchSelect {
  execProtected = {
...
describe('FormFieldJobCodeSearchSelectComponent', () => {
  beforeEach(() => {
  ...
  beforeEach(waitForAsync(() => {
    ...
    TestBed.configureTestingModule({
      declarations: [FormFieldJobCodeSearchSelect, TestHostComponent],
      imports: [
    ...
    testResult1 = new ApiJobCode({
      targetNodeName: 'Job Code 1',
      code: '1.101',
    });
    testResult2 = new ApiJobCode({
      targetNodeName: 'Job Code 2',
      code: '1.102',
    });
  ...
  it('areSearchResultsEqual should return correct result', () => {
    const copy = new ApiJobCode();
    expect(component.areSearchResultsEqual(testResult1, testResult1)).toEqual(
      true
    );
    expect(component.areSearchResultsEqual(testResult1, testResult2)).toEqual(
      false
    );
  });
  ...
  it('labelFor should return correct result', () => {
    expect(component.labelFor(testResult1)).toEqual(
      `${testResult1.code}|${testResult1.targetNodeName}`
    );
  });

  it('searchResultFromString should return correct result', () => {
    expect(
      component.searchResultFromString(testResult1.targetNodeName!)
    ).toEqual(undefined);
  });
