...
describe('form-field-jobcode-search-select.utilities', () => {
  ...
  it('areSearchResultsEqual should return correct result', () => {
    const copy = createMockJobcode(
      testJobCode1.targetNode,
      testJobCode1.targetNodeName,
      testJobCode1.code
    );
  ...
  it('labelFor should return correct result', () => {
    expect(labelFor(testJobCode1)).toEqual(
      `${testJobCode1.code}|${testJobCode1.targetNodeName}`
    );
  });

  it('searchResultFromString should return correct result', () => {
    expect(searchResultFromString(testJobCode1.targetNodeName!)).toEqual(
      undefined
    );
  });
});
