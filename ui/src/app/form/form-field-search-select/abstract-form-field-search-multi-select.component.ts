...
export abstract class AbstractFormFieldSearchMultiSelect<
  TValue,
> extends AbstractFormFieldSearchSelect<TValue, TValue[] | null> {
  ...
  isResultSelected(result: TValue): boolean {
    if (this.isSearchResultType(result)) {
      return !!this.controlValue?.find((r: TValue) =>
        this.areSearchResultsEqual(r, result)
      );
    }
  ...
  selectResult(result: TValue): void {
    if (this.isSearchResultType(result) && !this.isResultSelected(result)) {
      const selected: TValue[] = this.controlValue ?? [];
    ...
    if (this.isSearchResultType(result)) {
      const selected =
        this.controlValue?.filter(
          (r: TValue) => !this.areSearchResultsEqual(r, result)
        ) ?? [];
