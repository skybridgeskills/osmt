...
export abstract class AbstractFormFieldSearchSelect<TSearch, TValue>
  extends AbstractFormField<TValue | null>
  implements OnInit, OnDestroy
{
  ...
  ngOnInit() {
    this.searchControl.valueChanges.subscribe((v: string) =>
  ...
  get searchControlResultValue(): TSearch | undefined {
    ...
    if (searchResult) {
      const existingResult = this.results?.find((r: TSearch) =>
        this.areSearchResultsEqual(r, searchResult)
      );
      return (existingResult ?? this.createNonExisting)
        ? searchResult
        : undefined;
    }
  ...
  get showResults(): boolean {
    return (
      this.isSearchDirty && !this.isSearchEmpty && this.results !== undefined
    );
  }
  ...
  protected performSearch(text: string): void {
    ...
    this.queryInProgress = this.callSearchService(text).subscribe(
      searchResults => {
  ...
  protected selectSearchValue() {
    ...
    searchResult =
      (searchResult ?? this.createNonExisting)
        ? this.searchResultFromString(this.searchControlValue)
        : undefined;
  ...
  protected setSearchControlValue(value: string | null, emitEvent = true) {
