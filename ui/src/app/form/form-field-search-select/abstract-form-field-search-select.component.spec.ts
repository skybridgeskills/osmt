...
export abstract class TestHostComponent extends AbstractFormFieldSearchSelect<
  ITestValueType,
  ITestValueType
> {
  ...
  areSearchResultsEqual(
    result1: ITestValueType,
    result2: ITestValueType
  ): boolean {
    return result1?.value === result2?.value;
  }
  isSearchResultType(result: ITestValueType): result is ITestValueType {
    return true;
  }
  labelFor(result: ITestValueType): string | null {
    return result?.value ?? null;
  }
  protected callSearchService(text: string): Observable<ITestValueType[]> {
    return of([{ value: 'abc-123' } as ITestValueType]);
  }
  ...
  protected searchResultFromString(value: string): ITestValueType | undefined {
    return { value: value } as ITestValueType;
  }

  execProtected = {
    setSearchControlValue: (value: string | null, emitEvent = true) =>
      this.setSearchControlValue(value, emitEvent),
    ...
    onSearchValueChange: (newValue: string) =>
      this.onSearchValueChange(newValue),
...
describe('AbstractFormFieldSearchSelectComponent', () => {
  ...
  beforeEach(waitForAsync(() => {
    ...
    TestBed.configureTestingModule({
      declarations: [AbstractFormFieldSearchSelect, TestHostComponent],
      imports: [
