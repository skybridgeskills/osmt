...
export class AbstractTableComponent<SummaryT> implements OnInit {
  ...
  @Output() rowSelected: EventEmitter<SummaryT[]> = new EventEmitter<
    SummaryT[]
  >();
  ...
  getNameSort(): boolean | undefined {
    if (this.currentSort) {
      switch (this.currentSort) {
        case ApiSortOrder.NameAsc:
          return true;
        case ApiSortOrder.NameDesc:
          return false;
      }
    }
    return undefined;
  ...
  getSkillSort(): boolean | undefined {
    if (this.currentSort) {
      switch (this.currentSort) {
        case ApiSortOrder.SkillAsc:
          return true;
        case ApiSortOrder.SkillDesc:
          return false;
      }
    }
    return undefined;
  ...
  getSelectAllCount(): number {
    return this.selectAllCount !== undefined
      ? this.selectAllCount
      : this.items.length;
  }
  ...
  handleSelectAll(event: SelectAllEvent): void {
    ...
    const isAllResultsSelected: boolean =
      event.selected && event.value === SelectAll.SELECT_ALL;
