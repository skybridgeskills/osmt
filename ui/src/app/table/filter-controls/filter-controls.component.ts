...
export class FilterControlsComponent implements OnInit, OnChanges {
  ...
  @Output() keywordsChanged: EventEmitter<FilterDropdown> =
    new EventEmitter<FilterDropdown>();
  @Output() filtersChanged: EventEmitter<Set<PublishStatus>> = new EventEmitter<
    Set<PublishStatus>
  >();
  @Input()
  ...
  get isDraftChecked(): boolean | undefined {
    return this.isStatusChecked(PublishStatus.Draft);
  }
  get isPublishedChecked(): boolean | undefined {
    return this.isStatusChecked(PublishStatus.Published);
  }
  get isArchivedChecked(): boolean | undefined {
    return this.isStatusChecked(PublishStatus.Archived);
  }
