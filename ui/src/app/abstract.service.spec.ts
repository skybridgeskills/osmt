...
class Work implements IWork {
  // This just follows the pattern used throughout the source code
  foo: string;
...
describe('AbstractService (no HTTP needed)', () => {
  ...
  it('buildTableParams should be correct', () => {
    ...
    // Act
    const params = testService.buildTableParams(size, from, filter, sort);
...
describe('AbstractService (HTTP needed)', () => {
  ...
  it('buildUrl should return', () => {
    const path = 'data';
    expect(testService.buildUrl(path)).toEqual(
      AppConfig.settings.baseApiUrl + getBaseApi() + '/' + path
    );
  });
  ...
  it('buildUrl should works with tasks', () => {
    ...
    expect(builtUrlWithSlice).toEqual(
      AppConfig.settings.baseApiUrl + mockTaskResult.id
    );
  });
  ...
  it('bulkStatusChange should return', () => {
    ...
    // Act
    const result$ = testService.bulkStatusChange(path, apiSearch, newStatus);
  ...
  it('pollForTaskResult should return', done => {
    ...
    // Act
    testService
      .pollForTaskResult<IWork>(worker.doWork(path), 1000)
      .subscribe(data => {
        expect(data).toEqual(testData);
        done();
      });
