...
export class CategoryDetailCardComponent {
  ...
  get categorySkillsLabel(): string {
    let label =
      this.showSkillCount && this.categorySkillCount
        ? `${this.categorySkillCount} RSD${this.category?.skillCount != 1 ? 's' : ''} with category.`
        : '';

    label =
      this.firstSkillIndex && this.lastSkillIndex
        ? `${label} Viewing ${this.firstSkillIndex}-${this.lastSkillIndex}.`
        : label;
  ...
  get categorySkillCount(): string | undefined {
    return this.category?.skillCount
      ? this.category.skillCount.toString()
      : undefined;
  }
  ...
  get lastSkillIndex(): string | undefined {
    return this.indexOfFirstSkill && this.currentOnPage
      ? `${this.indexOfFirstSkill + this.currentOnPage}`
      : undefined;
  }
}
