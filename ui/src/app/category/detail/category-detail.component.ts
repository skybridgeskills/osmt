  ...
  implements OnInit
{
  ...
  get skillsViewingLabel(): string {
    return this.skillTableControl.currFirstSkillIndex &&
      this.skillTableControl.currLastSkillIndex
  ...
  protected setCategory(category: ApiCategory) {
    ...
    this.titleService.setTitle(
      `${category.name} | Category | ${this.whitelabel.toolName}`
    );
    this.loadSkills();
  }
  ...
  protected loadCategory() {
    if (this.idParam) {
      this.categoryService
        ...
        .subscribe((c: ApiCategory) => this.setCategory(c));
    } else {
      this.clearCategory();
    }
  }
