...
class CustomJobCodeRepositoryImpl
    @Autowired
    constructor(
        ...
        private fun multiPropertySearch(searchStr: String): Query {
            ...
            val nameMustExistQuery =
                QueryBuilders.exists { qt: ExistsQuery.Builder ->
                    qt.field(JobCode::name.name).boost(1.0f)
                }
            return QueryBuilders.bool { builder: BoolQuery.Builder ->
                builder.must(disMaxQuery, nameMustExistQuery).boost(1.0f)
            }
        }
        ...
        private fun createPrefixQueries(searchStr: String): List<Query> =
            listOf(
                OsmtQueryHelper.createPrefixDslQuery(
                    "${JobCode::code.name}.keyword",
                    searchStr,
                    2.0f,
                ),
                OsmtQueryHelper.createPrefixDslQuery(
                    "${JobCode::minorCode.name}.keyword",
                    searchStr,
                ),
                OsmtQueryHelper.createPrefixDslQuery(
                    "${JobCode::detailedCode.name}.keyword",
                    searchStr,
                ),
                OsmtQueryHelper.createPrefixDslQuery(
                    "${JobCode::majorCode.name}.keyword",
                    searchStr,
                ),
                OsmtQueryHelper.createPrefixDslQuery(
                    "${JobCode::broadCode.name}.keyword",
                    searchStr,
                ),
            )
        ...
        private fun createSimpleQueries(searchStr: String): List<Query> =
            listOf(
                OsmtQueryHelper.createMatchPhrasePrefixDslQuery(
                    JobCode::name.name,
                    searchStr,
                    2.0f,
                ),
        ...
        private fun createComplexQueries(searchStr: String): List<Query> =
            listOf(
                OsmtQueryHelper.createSimpleQueryDslQuery(
                    "${JobCode::name.name}.raw",
                    searchStr,
                    2.0f,
                ),
                OsmtQueryHelper.createSimpleQueryDslQuery(
                    "${JobCode::minor.name}.raw",
                    searchStr,
                    1.0f,
                    true,
                ...
                OsmtQueryHelper.createSimpleQueryDslQuery(
                    "${JobCode::detailed.name}.raw",
                    searchStr,
                    1.0f,
                ),
                OsmtQueryHelper.createSimpleQueryDslQuery(
                    "${JobCode::major.name}.raw",
                    searchStr,
                    1.0f,
                ),
                OsmtQueryHelper.createSimpleQueryDslQuery(
                    "${JobCode::broad.name}.raw",
                    searchStr,
                    1.0f,
                ),
            )
        ...
        if (isComplex) {
            disjunctionQuery.innerQueries().addAll(
                listOf(
                    ...
                    simpleQueryStringQuery(
                        query,
                    ).field("${path}${JobCode::detailed.name}.raw").defaultOperator(Operator.AND),
                    simpleQueryStringQuery(
                        query,
                    ).field("${path}${JobCode::major.name}.raw").defaultOperator(Operator.AND),
                    simpleQueryStringQuery(
                        query,
                    ).field("${path}${JobCode::broad.name}.raw").defaultOperator(Operator.AND),
                ),
            )
        } else {
            disjunctionQuery.innerQueries().addAll(
...
interface JobCodeEsRepo :
    ElasticsearchRepository<JobCode, Int>,
    CustomJobCodeRepository
