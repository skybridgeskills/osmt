...
object JobCodeBreakout {
    private val codePartDelimiter =
        "[-.]".toRegex() // used to split code on either hyphen (-) or period (.)

    private fun majorPart(code: String): String? =
        code
            .split(codePartDelimiter)
            .takeIf { it.isNotEmpty() }
            ?.let { it[0] }
            ?.takeIf { it.toIntOrNull() != null }

    private fun minorPart(code: String): String? =
        code
            .split(codePartDelimiter)
            .takeIf { it.size > 1 }
            ?.let { it[1] }
    ...
    private fun minorLeadingPart(code: String): String? =
        code
            .split(codePartDelimiter)
            .takeIf { it.size > 1 }
            ?.let { it[1] }
    ...
    private fun broadPart(code: String): String? =
        code
            .split(codePartDelimiter)
            .takeIf { it.size > 1 }
            ?.let { it[1] }
    ...
    private fun detailedPart(code: String): String? =
        code
            .split(codePartDelimiter)
            .takeIf { it.size > 1 }
            ?.let { it[1] }
    ...
    private fun jobRolePart(code: String): String? =
        code
            .split(codePartDelimiter)
            .takeIf { it.size > 2 }
            ?.let { it[2] }
    ...
    fun majorCode(code: String): String? =
        majorPart(code)
    ...
    fun broadCode(code: String): String? =
        broadPart(code)
            ?.takeIf { isValidInt(it) }
            ?.let { "${majorPart(code)}-${minorPart(code)}${it}0" }

    fun detailedCode(code: String): String? =
        detailedPart(code)
            ?.takeIf { isValidInt(it) }
            ?.let { "${majorPart(code)}-${minorPart(code)}${broadPart(code)}$it" }

    fun jobRoleCode(code: String): String? =
        jobRolePart(code)
            ?.let {
                "${
                    this.majorPart(
                ...
                }-${this.minorPart(code)}${this.broadPart(code)}${this.detailedPart(code)}.$it"
            }
