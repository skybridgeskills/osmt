...
object OsmtQueryHelper {
    @Deprecated(
        ...
        ReplaceWith("createNativeQuery"),
        DeprecationLevel.WARNING,
    )
    fun convertToNativeQuery(
        pageable: Pageable,
        dslFilter: co.elastic.clients.elasticsearch._types.query_dsl.Query?,
        nsqb: NativeSearchQueryBuilder,
        msgPrefix: String,
        log: Logger,
    ): Query {
    ...
    fun createNativeQuery(
        pageable: Pageable,
        dslFilter: co.elastic.clients.elasticsearch._types.query_dsl.Query?,
        dslQuery: co.elastic.clients.elasticsearch._types.query_dsl.Query,
        msgPrefix: String? = null,
        log: Logger? = null,
        ...
        if (nativeQuery.springDataQuery != null) {
            log?.debug(
                String.Companion.format(
                    "\t\t%s",
                    (nativeQuery.springDataQuery as StringQuery).source,
                ),
            )
        }
    ...
    fun createMatchPhrasePrefixDslQuery(
        fieldName: String,
        searchStr: String,
        boostVal: Float? = 1.0f,
    ): co.elastic.clients.elasticsearch._types.query_dsl.Query =
        QueryBuilders.matchPhrasePrefix { qb ->
            qb.field(fieldName).query(searchStr).boost(boostVal)
        }

    fun createMatchBoolPrefixDslQuery(
        fieldName: String,
        searchStr: String,
        boostVal: Float? = null,
    ): co.elastic.clients.elasticsearch._types.query_dsl.Query =
        QueryBuilders.matchBoolPrefix { qb ->
            qb.field(fieldName).query(searchStr).boost(boostVal)
        }

    fun createPrefixDslQuery(
        fieldName: String,
        searchStr: String,
        boostVal: Float? = 1.0f,
    ): co.elastic.clients.elasticsearch._types.query_dsl.Query =
        QueryBuilders.prefix { qb ->
            qb.field(fieldName).value(searchStr).boost(boostVal)
        }

    fun createSimpleQueryDslQuery(
        fieldName: String,
        searchStr: String,
        boostVal: Float? = null,
        wildCardFlag: Boolean? = false,
        operatorVal: Operator? = Operator.And,
    ): co.elastic.clients.elasticsearch._types.query_dsl.Query =
        QueryBuilders.simpleQueryString { qb ->
            qb
                .fields(fieldName)
                .query(searchStr)
                .boost(boostVal)
    ...
    fun createDisMaxDslQuery(
        queries: List<co.elastic.clients.elasticsearch._types.query_dsl.Query>,
        tieBreakerVal: Double? = 0.0,
        boostVal: Float? = 1.0f,
    ): co.elastic.clients.elasticsearch._types.query_dsl.Query =
        QueryBuilders.disMax { qb ->
            qb.queries(queries).tieBreaker(tieBreakerVal).boost(boostVal)
        }

    // TODO: Finish this in total elasticSearch conversion to 8.X story
    fun createNestedQueryDslQuery(
        path: String,
        _scoreMode: ChildScoreMode,
        query: co.elastic.clients.elasticsearch._types.query_dsl.Query? = null,
        innerHits: InnerHits? = null,
    ): co.elastic.clients.elasticsearch._types.query_dsl.Query {
    ...
    fun createTermsDslQuery(
        fieldName: String,
        filterValues: List<String>,
        andFlag: Boolean = true,
    ): co.elastic.clients.elasticsearch._types.query_dsl.Query {
        val values =
            filterValues
                .stream()
    ...
    fun createSort(
        ...
        SortOptions
            .Builder()
            .field { f ->
                f.field(fieldName).order(sortOrder)
            }.build()
}
