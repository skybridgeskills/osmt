...
import co.elastic.clients.elasticsearch._types.query_dsl.QueryBuilders.disMax
...
import org.elasticsearch.index.query.QueryBuilders.matchBoolPrefixQuery
...
import org.elasticsearch.index.query.QueryBuilders.prefixQuery
...
interface CustomRichSkillQueries : FindsAllByPublishStatus<RichSkillDoc> {
    ...
    fun generateBoolQueriesFromApiSearch(
        bq: BoolQueryBuilder,
        advancedQuery: ApiAdvancedSearch,
    )

    fun generateBoolQueriesFromApiSearchWithFilters(
        bq: BoolQueryBuilder,
        filteredQuery: ApiFilteredSearch,
        publishStatus: Set<PublishStatus>,
    )
...
class CustomRichSkillQueriesImpl
    @Autowired
    constructor(
        ...
        @Deprecated(
            "ElasticSearch 7.X has been deprecated",
            ReplaceWith("CustomJobCodeRepository.typeAheadSearch()"),
        ...
        @Deprecated(
            "ElasticSearch 7.X has been deprecated",
            ReplaceWith("buildNestedQueriesNu"),
            DeprecationLevel.WARNING,
        )
        private fun buildNestedQueries(
            path: String? = null,
         ...
         */
        private fun buildNestedQueriesNu(
            path: String? = null,
            queryParams: List<String>,
        ): Query {
        ...
        @Deprecated(
            "ElasticSearch 7.X has been deprecated",
            ReplaceWith("generateBoolQueriesFromApiSearchNu"),
            DeprecationLevel.WARNING,
        )
        override fun generateBoolQueriesFromApiSearch(
            ...
            with(advancedQuery) {
                    ...
                    if (it.contains("\"")) {
                        bq.must(
                            simpleQueryStringQuery(
                                it,
                            ).field("${RichSkillDoc::name.name}.raw").defaultOperator(Operator.AND),
                        )
                    } else {
                    ...
                    if (it.matches(Regex(QUOTED_SEARCH_REGEX_PATTERN))) {
                        bq.must(
                            simpleQueryStringQuery(
                                ...
                                "${RichSkillDoc::categories.name}.raw",
                    ...
                    if (it.contains("\"")) {
                        bq.must(
                            simpleQueryStringQuery(
                                ...
                                "${RichSkillDoc::statement.name}.raw",
                            ).defaultOperator(Operator.AND),
                        )
                    } else {
                ...
                keywords?.map {
                    if (it.contains("\"")) {
                        bq.must(
                            simpleQueryStringQuery(it)
                                .field("${RichSkillDoc::searchingKeywords.name}.raw")
                                .defaultOperator(Operator.AND),
                        )
                    } else {
                        ...
                        if (s.contains("\"")) {
                            bq.must(
                                simpleQueryStringQuery(
                                    ...
                                    "${RichSkillDoc::standards.name}.raw",
                                ).defaultOperator(Operator.AND),
                            )
                        } else {
                ...
                certifications?.let { it ->
                    it.mapNotNull { it.name }.map { s ->
                        if (s.contains("\"")) {
                            bq.must(
                                simpleQueryStringQuery(s)
                                    .field("${RichSkillDoc::certifications.name}.raw")
                                    .defaultOperator(Operator.AND),
                            )
                        } else {
                        ...
                        if (s.contains("\"")) {
                            bq.must(
                                simpleQueryStringQuery(
                                    ...
                                    "${RichSkillDoc::employers.name}.raw",
                                ).defaultOperator(Operator.AND),
                            )
                        } else {
        ...
        fun generateBoolQueriesFromApiSearchNu(advancedQuery: ApiAdvancedSearch): Query {
            with(advancedQuery) {
                return bool { bq ->
                    skillName.nullIfEmpty()?.let {
                        bq.must(
                            createQueryFromString(RichSkillDoc::name.name, it),
                        )
                    }
                    category.nullIfEmpty()?.let {
                        bq.must(
                            createQueryFromString(
                                RichSkillDoc::categories.name,
                                it,
                                QUOTED_SEARCH_REGEX_PATTERN,
                            ),
                        )
                    }
                    author.nullIfEmpty()?.let {
                        bq.must(
                            createQueryFromString(RichSkillDoc::authors.name, it),
                        )
                    }
                    skillStatement.nullIfEmpty()?.let {
                        bq.must(createQueryFromString(RichSkillDoc::statement.name, it))
                    }
                    keywords?.let {
                        bq.must(
                            createQueryFromStringList(RichSkillDoc::searchingKeywords.name, it),
                        )
                    }
// TODO implement this
//                occupations.nullIfEmpty()?.let { bq.must(createQueryFromString(RichSkillDoc::name.name, it)) }
                    ...
                    standards?.let {
                        bq.must(
                            createQueryFromApiNameList(RichSkillDoc::standards.name, it),
                        )
                    }
                    certifications?.let {
                        bq.must(
                            createQueryFromApiNameList(RichSkillDoc::certifications.name, it),
                        )
                    }
                    employers?.let {
                        bq.must(
                            createQueryFromApiNameList(RichSkillDoc::employers.name, it),
                        )
                    }
                    alignments?.let {
                        bq.must(
                            createQueryFromApiNameList(RichSkillDoc::alignments.name, it),
                        )
                    }
                }
            }
        ...
        private fun createQueryFromString(
            fieldName: String,
            searchStr: String,
            regEx: String? = null,
        ): Query {
            val isComplex =
                searchStr.contains("\"") || (regEx != null && searchStr.matches(Regex(regEx)))
            return if (isComplex) {
        ...
        private fun createQueryFromStringList(
            fieldName: String,
            searchStrList: List<String>,
        ): List<Query> =
            searchStrList
                .stream()
        ...
        private fun createQueryFromApiNameList(
            fieldName: String,
            searchStrList: List<ApiNamedReference>,
        ): List<Query> = createQueryFromStringList(fieldName, searchStrList.map { it.name ?: "" })

        @Deprecated(
            "ElasticSearch 7.X has been deprecated",
            ReplaceWith("generateBoolQueriesFromApiSearchWithFiltersNu"),
            DeprecationLevel.WARNING,
        )
        override fun generateBoolQueriesFromApiSearchWithFilters(
            bq: BoolQueryBuilder,
            filteredQuery: ApiFilteredSearch,
            publishStatus: Set<PublishStatus>,
        ) {
            ...
            with(filteredQuery) {
                ...
                standards?.let {
                    it.mapNotNull {
                        bq.must(
                            generateTermsSetQueryBuilder(RichSkillDoc::standards.name, standards),
                        )
                    }
                }
                certifications?.let {
                ...
                alignments?.let {
                    it.mapNotNull {
                        bq.must(
                            generateTermsSetQueryBuilder(RichSkillDoc::alignments.name, alignments),
                        )
                    }
                }
                employers?.let {
                    it.mapNotNull {
                        bq.must(
                            generateTermsSetQueryBuilder(RichSkillDoc::employers.name, employers),
                        )
                    }
                }
                authors?.let {
        ...
        @Deprecated(
            "ElasticSearch 7.X has been deprecated",
            ReplaceWith("generateTermsSetQueryBuilderNu"),
            DeprecationLevel.WARNING,
        )
        private fun generateTermsSetQueryBuilder(
            fieldName: String,
            list: List<String>,
            ...
            TermsSetQueryBuilder(
                "$fieldName.keyword",
                list,
            ).setMinimumShouldMatchScript(Script(list.size.toString()))

        /**
         * ElasticSearch v8.7.X version
         */
        private fun generateTermsSetQueryBuilderNu(
            fieldName: String,
            list: List<String>,
        ): Query {
            val sb =
                co.elastic.clients.elasticsearch._types.Script
                    .Builder()
                    .inline { il -> il.source(list.size.toString()) }
            return termsSet { qb ->
                qb
        ...
        @Deprecated(
            "ElasticSearch 7.X has been deprecated",
            ReplaceWith("richSkillPropertiesMultiMatchNu"),
            DeprecationLevel.WARNING,
        )
            ...
            val complexQueries =
                listOf(
                    simpleQueryStringQuery(query)
                        .field("${RichSkillDoc::name.name}.raw")
                        .boost(2.0f)
                        .defaultOperator(Operator.AND),
                    simpleQueryStringQuery(
                        query,
                    ).field("${RichSkillDoc::statement.name}.raw").defaultOperator(Operator.AND),
                    simpleQueryStringQuery(
                        query,
                    ).field("${RichSkillDoc::categories.name}.raw").defaultOperator(Operator.AND),
                    simpleQueryStringQuery(query)
                        .field("${RichSkillDoc::searchingKeywords.name}.raw")
                        .defaultOperator(Operator.AND),
                    simpleQueryStringQuery(
                        query,
                    ).field("${RichSkillDoc::standards.name}.raw").defaultOperator(Operator.AND),
                    simpleQueryStringQuery(query)
                        .field("${RichSkillDoc::certifications.name}.raw")
                        .defaultOperator(Operator.AND),
                    simpleQueryStringQuery(
                        query,
                    ).field("${RichSkillDoc::employers.name}.raw").defaultOperator(Operator.AND),
                    simpleQueryStringQuery(
                        query,
                    ).field("${RichSkillDoc::alignments.name}.raw").defaultOperator(Operator.AND),
                    simpleQueryStringQuery(
                        query,
                    ).field("${RichSkillDoc::authors.name}.raw").defaultOperator(Operator.AND),
                )
        ...
        private fun buildQuery(
            ...
            if (!apiSearch.query.isNullOrBlank()) {
                if (collectionId.isNullOrBlank()) {
                    ...
                    bq.should(
                        nestedQuery(
                            ...
                            simpleQueryStringQuery(apiSearch.query)
                                .field("collections.name.raw")
                                .defaultOperator(Operator.AND),
                            ScoreMode.Avg,
                        ),
                    )
                    ...
                    if (apiSearch.filtered != null) {
                        ...
                        bq.must(
                            nestedQuery(
                                RichSkillDoc::collections.name,
                                boolQuery().must(
                                    matchQuery(collectionsUuid, collectionId).operator(Operator.OR),
                                ),
                                ScoreMode.Avg,
                            ),
                        )
                    ...
                    bq.must(
                        BoolQueryBuilder()
                            .should(richSkillPropertiesMultiMatch(apiSearch.query))
                    ...
                    apiSearch.advanced?.collectionName?.let {
                        bq.must(
                            nestedQuery(
                                ...
                                simpleQueryStringQuery(
                                    it,
                                ).field("collections.name.raw").defaultOperator(Operator.AND),
                                ScoreMode.Avg,
                            ),
                        )
                    }
                ...
                var apiSearchUuids =
                    apiSearch.uuids?.filterNotNull()?.filter { x: String? ->
                        x !=
                            ""
                    }
...
interface RichSkillEsRepo :
    ElasticsearchRepository<RichSkillDoc, Int>,
    CustomRichSkillQueries {
    fun findByUuid(
        uuid: String,
        pageable: Pageable =
