...
import org.springframework.data.elasticsearch.annotations.FieldType.Search_As_You_Type
    ...
    companion object {
        fun fromDao(
            ...
            RichSkillDocV2(
                ...
                category =
                    dao.keywords
                        .filter { it.type == KeywordTypeEnum.Category }
                        .mapNotNull { it.value }
                        .sorted()
                        .joinToString(SEMICOLON),
                author =
                    dao.keywords
                        .filter { it.type == KeywordTypeEnum.Author }
                        .mapNotNull { it.value }
                        .sorted()
                        .joinToString(SEMICOLON),
                ...
                searchingKeywords =
                    dao.keywords
                        .filter {
                            it.type == KeywordTypeEnum.Keyword
                        }.mapNotNull { it.value },
                jobCodes = dao.jobCodes.map { it.toModel() },
                standards =
                    dao.keywords
                        .filter {
                            it.type == KeywordTypeEnum.Standard
                        }.mapNotNull { it.value },
                certifications =
                    dao.keywords
                        .filter { it.type == KeywordTypeEnum.Certification }
                        .mapNotNull { it.value },
                employers =
                    dao.keywords
                        .filter {
                            it.type == KeywordTypeEnum.Employer
                        }.mapNotNull { it.value },
                alignments =
                    dao.keywords
                        .filter { it.type == KeywordTypeEnum.Alignment }
                        .mapNotNull { it.value },
