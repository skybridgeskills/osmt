...
interface RichSkillRepository : PaginationHelpers<RichSkillDescriptorTable> {
    ...
    fun createFromApi(
        skillUpdates: List<ApiSkillUpdate>,
        user: String,
        userEmail: String,
    ...
    constructor(
        ...
        override fun update(
            ...
            val (publishStatusChanges, otherChanges) =
                daoObject
                    ?.toModel()
                    ?.diff(old)
                    ?.partition { it.fieldName == RichSkillDescriptor::publishStatus.name }
                    ?: (null to null)
        ...
        override fun updateFromApi(
            ...
            val updateObjectWithId =
                rsdUpdateObject.copy(
                    id = existingSkillId,
                )
            return update(updateObjectWithId, user)
        }

        override fun rsdUpdateFromApi(
            skillUpdate: ApiSkillUpdate,
            user: String,
            email: String,
        ): RsdUpdateObject {
            ...
            skillUpdate.collections?.let { slu ->
                slu.add
                    ?.mapNotNull {
                        collectionRepository.findByName(it)
                            ?: collectionRepository.create(it, user, email)
                    }?.let {
            ...
            fun lookupReferences(
                ...
                lud.remove
                    ?.map {
                        keywordRepository.findByValueOrUri(
                            keywordType,
                            value = it.name,
                            uri = it.id,
                        )
            ...
            fun lookupAlignments(
                lud: ApiAlignmentListUpdate,
                keywordType: KeywordTypeEnum,
            ) {
                lud.add
                    ?.map {
            ...
            return RsdUpdateObject(
                ...
                collections =
                    if (addingCollections.size + removingCollections.size > 0) {
                        ListFieldUpdate(
                            addingCollections,
                            removingCollections,
                        )
        ...
        override fun changeStatusesForTask(publishTask: PublishTask): ApiBatchResult {
            ...
            val publishSkill = { skillDao: RichSkillDescriptorDao, task: PublishTask ->
                ...
                if (oldStatus != task.publishStatus) {
                    val updateObj =
                        RsdUpdateObject(id = skillDao.id.value, publishStatus = task.publishStatus)
        ...
        override fun containingJobCode(jobCode: String): SizedIterable<RichSkillDescriptorDao> {
            val query =
                RichSkillDescriptorTable
                    .innerJoin(RichSkillJobCodes)
                    .innerJoin(JobCodeTable)
                    .slice(RichSkillDescriptorTable.columns)
                    .select {
