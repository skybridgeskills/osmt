...
class CollectionRepositoryImpl
    ...
    constructor(
        ...
        override fun create(
            ...
            newCollection.let {
                ...
                richSkillEsRepo.saveAll(
                    it.skills.map { skill ->
                        RichSkillDoc.fromDao(skill, appConfig)
                    },
                )
            }
        ...
        override fun update(
            ...
            val skillDaosToTrack =
                (
                    updateObject.skills?.add.orEmpty() +
                        updateObject.skills?.remove.orEmpty() +
                        currentSkillDaos
                ).distinct()
            ...
            val (publishStatusChanges, otherChanges) =
                daoObject
                    ?.toModel()
                    ?.diff(oldObject)
        ...
        override fun remove(uuid: String): ApiBatchResult {
            ...
            val esCollectionFound =
                collectionFound?.let {
                    collectionEsRepo.findByUuid(it.uuid, PageRequest.of(0, PaginationDefaults.size))
                }
        ...
        override fun updateFromApi(
            ...
            val collectionUpdateObject =
                collectionUpdateObjectFromApi(collectionUpdate, richSkillRepository)

            val updateObjectWithId =
                collectionUpdateObject.copy(
        ...
        override fun updateSkillsForTask(
            ...
            fun addSkillDao(skillDao: RichSkillDescriptorDao?) {
                skillDao?.let { dao ->
                    modifiedSkillDaos
                        .add(
                            dao,
                        ).also { modifiedSkillOriginals.put(dao.uuid, dao.toModel()) }
            ...
            if (!task.skillListUpdate.add
                ...
                totalCount += task.skillListUpdate.add
                    ?.uuids
                    ?.size ?: 0
            ...
            // process removals
            if (!task.skillListUpdate.remove
                    ?.uuids
                    .isNullOrEmpty()
            ) {
            ...
            return CollectionUpdateObject(
                ...
                skills =
                    if (adding.size + removing.size >
                        ...
                        ListFieldUpdate(adding, removing)
                    } else {
                        null
                    },
            )
        ...
        override fun changeStatusesForTask(publishTask: PublishTask): ApiBatchResult? {
            ...
            fun handleCollectionDao(collectionDao: CollectionDao?) {
                collectionDao?.let {
