...
object TestObjectHelpers {
    ...
    fun randomCollectionDoc(): CollectionDoc = collectionDoc(name = randomString(), description = randomString(), author = randomString())
    ...
    fun richSkillDoc(
        ...
        return RichSkillDoc(
            ...
            collections =
                listOf(
                    collectionDoc(elasticIdCounter, UUID.randomUUID().toString(), randomString()),
                ),
        )
    }
    ...
    fun apiSkillUpdateGenerator(
        ...
        val alignments =
            ApiAlignmentListUpdate(
                add =
                    (1..alignmentCount).toList().map {
                        alignmentGenerator(includeName = false, includeUri = true)
                    },
            )
        val employers =
            ApiReferenceListUpdate(
                add =
                    (1..employerCount).toList().map {
                        namedReferenceGenerator(includeName = true, includeUri = false)
                    },
            )
        val occupations =
            ApiStringListUpdate(
    ...
    fun assertThatKeywordMatchesNamedReference(
        keyword: Keyword?,
        namedReference: ApiNamedReference?,
    ) {
    ...
    fun assertThatKeywordMatchesAlignment(
        keyword: Keyword?,
