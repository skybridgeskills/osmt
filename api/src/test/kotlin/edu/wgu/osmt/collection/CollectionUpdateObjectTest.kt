...
@Transactional
internal class CollectionUpdateObjectTest
    @Autowired
    constructor(
        ...
        fun `applyStatusChange() should apply Archived status`() {
            ...
            val collectionUpdateObject =
                CollectionUpdateObject(publishStatus = PublishStatus.Archived)
        ...
        fun `applyStatusChange() should apply Published status`() {
            // Arrange
            val collectionUpdateObject =
                CollectionUpdateObject(publishStatus = PublishStatus.Published)
            val dao: CollectionDao =
                collectionRepository.create(
                    name = "name",
                    user = "user",
        ...
        fun `applyStatusChange() should return to previous draft status if no Publish date is recorded`() {
            // Arrange
            val collectionUpdateObject =
                CollectionUpdateObject(publishStatus = PublishStatus.Unarchived)
            val dao: CollectionDao =
                collectionRepository.create(
                    name = "name",
                    user = "user",
        ...
        fun `applyStatusChange() should Apply workspace status`() {
            // Arrange
            val collectionUpdateObject =
                CollectionUpdateObject(publishStatus = PublishStatus.Workspace)
            val dao: CollectionDao =
                collectionRepository.create(
                    name = "name",
                    user = "user",
