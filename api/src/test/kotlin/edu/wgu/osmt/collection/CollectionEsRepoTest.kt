...
class CollectionEsRepoTest
    ...
    constructor(
        ...
        fun queryCollectionHits(query: String): List<CollectionDoc> =
            collectionEsRepo.byApiSearch(ApiSearch(query)).searchHits.map {
                it.content
            }
        ...
        fun `Should be able to query collections across multiple fields`() {
            ...
            fun assertAgainstCollectionDoc(cs: List<CollectionDoc>) {
                cs.map {
                    Assertions
                        .assertThat(
                            it.uuid,
                        ).isIn(elasticRichSkillDoc.collections.map { it.uuid })
                }
            ...
            Assertions
                .assertThat(
                    searchByCollectionDescription[0].uuid,
                ).isEqualTo(collectionDoc.uuid)
            ...
            assertAgainstCollectionDoc(
                queryCollectionHits(
                    elasticRichSkillDoc.authors[elasticRichSkillDoc.authors.indices.random()],
                ),
            )
            assertAgainstCollectionDoc(queryCollectionHits(elasticRichSkillDoc.statement))
            assertAgainstCollectionDoc(
                queryCollectionHits(
                    elasticRichSkillDoc.searchingKeywords[
                        elasticRichSkillDoc.searchingKeywords.indices
                            .random(),
                    ],
                ),
            )
            assertAgainstCollectionDoc(
                queryCollectionHits(
                    elasticRichSkillDoc.categories[elasticRichSkillDoc.categories.indices.random()],
                ),
            )
            assertAgainstCollectionDoc(
                queryCollectionHits(
                    elasticRichSkillDoc.jobCodes[elasticRichSkillDoc.jobCodes.indices.random()].code,
                ),
            )
            assertAgainstCollectionDoc(
                queryCollectionHits(
                    elasticRichSkillDoc.standards[elasticRichSkillDoc.standards.indices.random()],
                ),
            )
            assertAgainstCollectionDoc(
                queryCollectionHits(
                    elasticRichSkillDoc.certifications[
                        elasticRichSkillDoc.certifications.indices
                            .random(),
                    ],
                ),
            )
            assertAgainstCollectionDoc(
                queryCollectionHits(
                    elasticRichSkillDoc.employers[elasticRichSkillDoc.employers.indices.random()],
                ),
            )
        }
        ...
        fun `Should perform advanced quoted searches`() {
            ...
            val advancedQuotedSearch =
                collectionEsRepo
                    .byApiSearch(
                        ApiSearch(
                            advanced = ApiAdvancedSearch(collectionName = "\"Self-Management\""),
                        ),
                    ).searchHits
                    .map {
                        it.content
        ...
        fun `Should limit skill results to skills within a collection when collection id is present`() {
            ...
            val richskill1 =
                TestObjectHelpers.randomRichSkillDoc().copy(
                    name = "Skill associated with collection",
                    collections = listOf(collection),
                    jobCodes = listOf(jobcode),
                )
            val richskill2 =
                TestObjectHelpers.randomRichSkillDoc().copy(
                    name = "Skill not associated with collection",
                    collections = listOf(),
                    jobCodes = listOf(jobcode),
                )
            ...
            val unassociatedRichSkills =
                (1..10).map {
                    TestObjectHelpers.randomRichSkillDoc().copy(collections = listOf(collection))
                }

            collection =
                collection.copy(skillIds = associatedRichSkills.map { it.uuid } + richskill1.uuid)

            collectionEsRepo.save(collection)
            richSkillEsRepo.saveAll(
                associatedRichSkills + unassociatedRichSkills + richskill1 + richskill2,
            )

            val result =
                richSkillEsRepo
                    .byApiSearch(
                        ApiSearch(query = jobcode.name),
