...
class MockData {
    ...
    fun getRichSkillDocs(): List<RichSkillDoc> =
        richSkillDescriptor.values
            .stream()
            .map { rsd -> richSkillDescriptor2RichSkillDoc(rsd, appConfig) }
            .collect(Collectors.toList())

    fun getRichSkillDescriptors(): List<RichSkillDescriptor> = richSkillDescriptor.values.toList()

    fun getCollectionDocs(): List<CollectionDoc> = collections.values.stream().collect(Collectors.toList())

    fun getJobCodes(): List<JobCode> = jobCodes.values.stream().collect(Collectors.toList())

    fun getKeywords(): List<Keyword> = keywords.values.stream().collect(Collectors.toList())
     ...
     * @return [RichSkillDoc]
     */
    fun getRichSkillDoc(skillId: String?): RichSkillDoc = richSkillDescriptor2RichSkillDoc(richSkillDescriptor[skillId], appConfig)
    ...
    fun getCollectionDoc(collectionId: String): CollectionDoc? {
        val ret =
            collections.values
                .stream()
                .filter { (_, uuid) -> uuid == collectionId }
                .findAny()
    ...
    fun richSkillDescriptor2RichSkillDoc(
        ...
        RichSkillDoc(
            ...
            alignments =
                rsd.keywords
                    .filter {
                        it.type == KeywordTypeEnum.Alignment
                    }.mapNotNull { it.value },
    ...
    private fun richSkillDescriptor2RichSkillRow(rsd: RichSkillDescriptor?): RichSkillRow {
        val sep = "; "
        ...
        richSkillRow.skillCategories =
            rsd?.categories?.map { it.value }?.joinToString(separator = sep)
        ...
        richSkillRow.certifications =
            rsd?.certifications?.map { it.value }?.joinToString(separator = sep)
        richSkillRow.blsMajors =
            rsd
                ?.jobCodes
                ?.map { it.majorCode }
                ?.toMutableList()
        ...
        richSkillRow.alignmentTitle =
            rsd?.alignments?.map { it.value }?.joinToString(separator = sep)
        richSkillRow.alignmentUri = rsd?.alignments?.map { it.uri }?.joinToString(separator = sep)

        return richSkillRow
    }

    private fun jobCode2BlsJobCode(bls: JobCode): BlsJobCode {
    ...
    private fun jobCode2OnetJobCode(onet: JobCode): OnetJobCode {
