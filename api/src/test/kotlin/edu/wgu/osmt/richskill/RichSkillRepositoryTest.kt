...
class RichSkillRepositoryTest
    ...
    constructor(
        ...
        fun assertThatKeywordsMatchStringList(
            keywords: List<Keyword>,
            stringList: ApiStringListUpdate,
        ) {
        ...
        fun assertThatCollectionsMatchStringList(
        ...
        fun assertThatJobCodesMatchStringList(
        ...
        fun assertThatKeywordsMatchReferenceList(
            keywords: List<Keyword>,
            referenceList: ApiReferenceListUpdate,
        ) {
            referenceList.add?.forEach { namedReference ->
                val found =
                    keywords.find {
                        it.value == namedReference.name &&
                            it.uri == namedReference.id
                    }
                assertThat(found).isNotNull
        ...
        fun assertThatKeywordsMatchAlignmentList(
            keywords: List<Keyword>,
            referenceList: ApiAlignmentListUpdate,
        ) {
        ...
        fun assertThatRichSkillMatchesApiSkillUpdate(
            rsc: RichSkillAndCollections,
            apiObj: ApiSkillUpdate,
        ) {
            val skill = rsc.rs
        ...
        fun `should update an existing skill from an ApiSkillUpdate object`() {
            ...
            newSkillUpdate =
                newSkillUpdate.copy(
                    keywords =
                        newSkillUpdate.keywords?.copy(
                            remove = originalSkillUpdate.keywords?.add,
                        ),
                    certifications =
                        newSkillUpdate.certifications?.copy(
                            remove = originalSkillUpdate.certifications?.add,
                        ),
                    standards =
                        newSkillUpdate.standards?.copy(
                            remove = originalSkillUpdate.standards?.add,
                        ),
                    alignments =
                        newSkillUpdate.alignments?.copy(
                            remove = originalSkillUpdate.alignments?.add,
                        ),
                    employers =
                        newSkillUpdate.employers?.copy(
                            remove = originalSkillUpdate.employers?.add,
                        ),
                    occupations =
                        newSkillUpdate.occupations?.copy(
                            remove = originalSkillUpdate.occupations?.add,
                        ),
                    collections =
                        newSkillUpdate.collections?.copy(
                            remove = originalSkillUpdate.collections?.add,
                        ),
                )

            val updatedDao =
                richSkillRepository.updateFromApi(
            ...
            assertThatRichSkillMatchesApiSkillUpdate(
                RichSkillAndCollections.fromDao(updatedDao!!),
                newSkillUpdate,
            )
        }
        ...
        fun `should create skills from ApiSkillUpdate objects`() {
            ...
            val results: List<RichSkillDescriptorDao> =
        ...
        fun `should not create a rich skill without a non-blank name and statement`() {
            ...
            assertThat(
                richSkillRepository.create(RsdUpdateObject(name = "name"), userString),
            ).isNull()
            assertThat(
                richSkillRepository.create(RsdUpdateObject(statement = "statement"), userString),
            ).isNull()
            assertThat(
                richSkillRepository.create(
                    RsdUpdateObject(name = " ", statement = " "),
                    userString,
                ),
            ).isNull()
        }
        ...
        fun `should be able to set an existing category to null on a skill via the API`() {
            ...
            val createObject =
                RsdUpdateObject(
                    name = name,
                    statement = statement,
                    keywords = ListFieldUpdate(add = listOf(category)),
                )
        ...
        fun `should be able to bulk publish all pages of search results`() {
            ...
            val skillUpdates =
                (1..totalSkillCount - toPublishCount).toList().map {
                    apiSkillUpdateGenerator()
                }
            val knownUpdates =
                (1..toPublishCount).toList().map {
                    apiSkillUpdateGenerator(
            ...
            val batchResult =
                richSkillRepository.changeStatusesForTask(
                    PublishTask(
        ...
        fun `should be able to bulk publish some skills within a collection`() {
            ...
            val archivedSkills =
                (1..archivedCount).toList().flatMap {
                    richSkillRepository
                        .createFromApi(
                            listOf(
                                apiSkillUpdateGenerator(publishStatus = PublishStatus.Published),
                            ),
                            userString,
                            userEmail,
                        ).map { skillDao ->
            ...
            val publishedSkills =
                (1..publishedCount).toList().flatMap {
                    richSkillRepository.createFromApi(
            ...
            val extraSkills =
                (1..notIncludedCount).toList().flatMap {
                    richSkillRepository.createFromApi(
                        listOf(apiSkillUpdateGenerator(publishStatus = PublishStatus.Draft)),
                        userString,
                        userEmail,
            ...
            publishedSkills.forEach {
                assertThat(
                    it.publishStatus(),
                ).isEqualTo(PublishStatus.Published)
            }
            archivedSkills.forEach {
                assertThat(
                    it.publishStatus(),
                ).isEqualTo(PublishStatus.Archived)
            }
            draftSkills.forEach { assertThat(it.publishStatus()).isEqualTo(PublishStatus.Draft) }
            deletedSkills.forEach {
                assertThat(
                    it.publishStatus(),
                ).isEqualTo(PublishStatus.Deleted)
            }
            ...
            collectionRepository.update(
                CollectionUpdateObject(
                    ...
                    skills =
                        ListFieldUpdate(
                            add = (
                                archivedSkills + publishedSkills + draftSkills +
                                    deletedSkills
                            ),
                        ),
                ),
        ...
        fun `should find rich skills by jobcode`() {
            ...
            val skillUpdate =
                apiSkillUpdateGenerator().copy(
                    occupations = ApiStringListUpdate(add = listOf(jobCode.code)),
                )
