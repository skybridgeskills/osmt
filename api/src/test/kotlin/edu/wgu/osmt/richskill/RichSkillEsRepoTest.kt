...
interface QuotedSearchHelpers {
    ...
    data class SearchSetupResults(
        val collections: List<CollectionDoc>,
        val skills: List<RichSkillDoc>,
    )

    fun quotedSearchSetup(): SearchSetupResults {
        var collection1 =
            TestObjectHelpers.collectionDoc(
                name = "Self-Management Collection",
                description = "Collection of Self-Management Skills",
            )
        val collection2 =
            TestObjectHelpers.collectionDoc(
                name = "Best Self Management Collection",
                description = "Collection of the best Self Management Skills",
            )
        ...
        val skill2 =
            TestObjectHelpers.richSkillDoc(
        ...
        return SearchSetupResults(
            listOf(collection1, collection2),
            listOf(skill1, skill2, skill3, skill4),
        )
    }
}

@Transactional
class RichSkillEsRepoTest
    @Autowired
    constructor(
        ...
        fun `Should update a rich skill in elastic search`() {
            ...
            val updateObject =
                RsdUpdateObject(
                    ...
                    keywords =
                        ListFieldUpdate(
                            add = listOf(keywordToAddDao!!),
                            remove = listOf(keywordToRemove),
                        ),
                )
            ...
            assertThat(
                esRetrieved.searchingKeywords,
            ).doesNotContain(keywordToRemove.toModel().value)
        ...
        fun queryRichSkillHits(query: String): List<RichSkillDoc> =
            richSkillEsRepo.byApiSearch(ApiSearch(query)).searchHits.map {
                it.content
            }
        ...
        fun `Should be able to query rich skills across multiple fields`() {
            ...
            assertAgainstRichSkillDoc(
                queryRichSkillHits(richSkillDoc.authors[richSkillDoc.authors.indices.random()]),
            )
            assertAgainstRichSkillDoc(queryRichSkillHits(richSkillDoc.statement))
            assertAgainstRichSkillDoc(
                queryRichSkillHits(
                    richSkillDoc.searchingKeywords[richSkillDoc.searchingKeywords.indices.random()],
                ),
            )

            assertAgainstRichSkillDoc(
                queryRichSkillHits(
                    richSkillDoc.categories[richSkillDoc.categories.indices.random()],
                ),
            )
            assertAgainstRichSkillDoc(
                queryRichSkillHits(
                    richSkillDoc.jobCodes[richSkillDoc.jobCodes.indices.random()].code,
                ),
            )
            assertAgainstRichSkillDoc(
                queryRichSkillHits(richSkillDoc.standards[richSkillDoc.standards.indices.random()]),
            )
            assertAgainstRichSkillDoc(
                queryRichSkillHits(
                    richSkillDoc.certifications[richSkillDoc.certifications.indices.random()],
                ),
            )
            assertAgainstRichSkillDoc(
                queryRichSkillHits(
            ...
            assertAgainstRichSkillDoc(
                queryRichSkillHits(richSkillDoc.employers[richSkillDoc.employers.indices.random()]),
            )
        ...
        fun `Should limit rich skills search to a related collection if collection id present`() {
            val associatedCollection =
                TestObjectHelpers.collectionDoc(
                    name = "Test Collection",
                    description = "Collection of Test Skills",
                )

            val skillsWCollectionIds =
                (1..100).map {
                    val doc = TestObjectHelpers.randomRichSkillDoc()
                    doc.copy(collections = listOf(associatedCollection))
                }

            val nonCollectionSkills =
                (1..100).map {
            ...
            val collectionRelatedResults =
                richSkillEsRepo.byApiSearch(
                    apiSearch = ApiSearch(),
                    collectionId = associatedCollection.uuid,
                )

            val allResults = richSkillEsRepo.byApiSearch(apiSearch = ApiSearch())

            val collectionRelatedSkillIds =
                collectionRelatedResults.searchHits
                    .map { it.content.uuid }
                    .toSet()
            val allResultsCollectionIds = allResults.searchHits.map { it.content.uuid }.toSet()

            assertThat(
                collectionRelatedSkillIds,
            ).isEqualTo(skillsWCollectionIds.map { it.uuid }.toSet())

            assertThat(allResultsCollectionIds).isEqualTo(
                (skillsWCollectionIds + nonCollectionSkills)
                    .map { it.uuid }
                    .toSet(),
            )
        }
        ...
        fun `Should match on partial word queries`() {
            var collection =
                TestObjectHelpers
                    .randomCollectionDoc()
                    .copy(
                        name = "A collection that questions the ability to perform partial query matches",
                    )
            val skill =
                TestObjectHelpers.randomRichSkillDoc().copy(
            ...
            val otherSkills =
                (1..100).map {
                    TestObjectHelpers.randomRichSkillDoc().copy(collections = otherCollections)
                }
            otherCollections =
                otherCollections.map { it.copy(skillIds = otherSkills.map { it.uuid }) }
            ...
            assertThat(
                skillSearchResult.searchHits
                    .first()
                    .content.uuid,
            ).isEqualTo(skill.uuid)
            assertThat(skillSearchResult.totalHits).isEqualTo(1L)

            assertThat(
                collectionSearchResult.searchHits
                    .first()
                    .content.uuid,
            ).isEqualTo(collection.uuid)
        ...
        fun `Should allow prefix searches on advanced search job codes`() {
            ...
            val jobCodes =
                listOf(
                    TestObjectHelpers.randomJobCode().copy(
                        code = "10-9999.88",
                        id = TestObjectHelpers.elasticIdCounter,
                    ),
                )

            val skillWithJobCodes =
                TestObjectHelpers.randomRichSkillDoc().copy(jobCodes = jobCodes).also {
                    richSkillEsRepo.save(it)
                }
            ...
            assertThat(
                searchResult.searchHits
                    .first()
                    .content.uuid,
            ).isEqualTo(skillWithJobCodes.uuid)
        }
        ...
        fun `Should allow prefix searches on keywords`() {
            ...
            val searchingKeywords =
                listOf("SEL: Interpersonal Communication", "Doing", "Verbal Communication Skills")
            ...
            val searchResult =
                richSkillEsRepo.byApiSearch(
                    ApiSearch(advanced = ApiAdvancedSearch(keywords = listOf("doing"))),
                )
            val searchResult2 =
                richSkillEsRepo.byApiSearch(
                    ApiSearch(advanced = ApiAdvancedSearch(keywords = listOf("sel"))),
                )
            val searchResult3 =
                richSkillEsRepo.byApiSearch(
                    ApiSearch(advanced = ApiAdvancedSearch(keywords = listOf("nada"))),
                )
            ...
            assertThat(
                searchResult.searchHits
                    .first()
                    .content.uuid,
            ).isEqualTo(skillWithKeywords.uuid)
            assertThat(
                searchResult2.searchHits
                    .first()
                    .content.uuid,
            ).isEqualTo(skillWithKeywords.uuid)
        ...
        fun `Should allow advanced searches with lists`() {
            ...
            assertThat(
                result1.searchHits
                    .first()
                    .content.uuid,
            ).isEqualTo(richSkill1.uuid)
            assertThat(
                result2.searchHits
                    .first()
                    .content.uuid,
            ).isEqualTo(richSkill2.uuid)
            assertThat(
                result3.searchHits
                    .first()
                    .content.uuid,
            ).isEqualTo(richSkill3.uuid)
            assertThat(
                result4.searchHits
                    .first()
                    .content.uuid,
            ).isEqualTo(richSkill4.uuid)
        }
        ...
        fun `Should perform simple quoted searches`() {
            ...
            val ambiguousNonQuotedSearch =
                richSkillEsRepo
                    .byApiSearch(
                        ApiSearch(query = "self management"),
                    ).searchHits
                    .map {
                        it.content
                    }
        ...
        fun `Should perform advanced quoted searches`() {
            ...
            val advancedQuotedSearch =
                richSkillEsRepo
                    .byApiSearch(
                        ApiSearch(advanced = ApiAdvancedSearch(skillName = "\"Self-Management\"")),
                    ).searchHits
                    .map {
                        it.content
                    }
        ...
        fun testByApiSearchWithAdvance() {
            ...
            val authorsList =
                listOf("Author1", "Author2", "Author3", "Author4", "Author5", "Author6")
            ...
            val skillByStatement =
                TestObjectHelpers.randomRichSkillDoc().copy(
                    statement = "skillStatement",
                )
            val skillByKeywords =
                TestObjectHelpers.randomRichSkillDoc().copy(
                    searchingKeywords = keywordslist,
                )
            val skillByStandards =
                TestObjectHelpers.randomRichSkillDoc().copy(
                    standards = keywordslist,
                )
            val skillByCertifications =
                TestObjectHelpers.randomRichSkillDoc().copy(
                    certifications = keywordslist,
                )
            val skillByEmployers =
                TestObjectHelpers.randomRichSkillDoc().copy(
                    employers = keywordslist,
                )
            val skillByAlignments =
                TestObjectHelpers.randomRichSkillDoc().copy(
                    alignments = keywordslist,
                )

            richSkillEsRepo.saveAll(
                listOf(
                    skillByName,
                    skillByCategory,
                    skillByAuthor,
            ...
            // Assert
            assertThat(
                skillResult.searchHits
                    .first()
                    .content.uuid,
            ).isEqualTo(skillByName.uuid)
            assertThat(
                categoryResult.searchHits
                    .first()
                    .content.uuid,
            ).isEqualTo(skillByCategory.uuid)
            assertThat(
                authorResult.searchHits
                    .first()
                    .content.uuid,
            ).isEqualTo(skillByAuthor.uuid)
            assertThat(
                skillStatementResult.searchHits
                    .first()
                    .content.uuid,
            ).isEqualTo(skillByStatement.uuid)
            assertThat(
                keywordsResult.searchHits
                    .first()
                    .content.uuid,
            ).isEqualTo(skillByKeywords.uuid)
            assertThat(
                standardsResult.searchHits
                    .first()
                    .content.uuid,
            ).isEqualTo(skillByStandards.uuid)
            assertThat(
                certificationsResult.searchHits
                    .first()
                    .content.uuid,
            ).isEqualTo(skillByCertifications.uuid)
            assertThat(
                employersResult.searchHits
                    .first()
                    .content.uuid,
            ).isEqualTo(skillByEmployers.uuid)
            assertThat(
                alignmentsResult.searchHits
                    .first()
                    .content.uuid,
            ).isEqualTo(skillByAlignments.uuid)
        }
        ...
        fun `search with categories filter should apply to filtered search`() {
            // Arrange
            val skillWithCategory1 =
                TestObjectHelpers.randomRichSkillDoc().copy(
                    categories = listOf("category1"),
                )
            val skillWithCategory2 =
                TestObjectHelpers.randomRichSkillDoc().copy(
                    categories = listOf("category2"),
                )
            val skillWithCategory3 =
                TestObjectHelpers.randomRichSkillDoc().copy(
                    categories = listOf("category3"),
                )

            richSkillEsRepo.saveAll(
                listOf(skillWithCategory1, skillWithCategory2, skillWithCategory3),
            )

            // Act
            val filteredSearchResult =
                richSkillEsRepo.byApiSearch(
                    ApiSearch(
                        filtered =
                            ApiFilteredSearch(
            ...
            // Assert
            assertThat(
                filteredSearchResult.searchHits
                    .first()
                    .content.uuid,
            ).isEqualTo(skillWithCategory1.uuid)
            assertThat(
                filteredSearchResult.searchHits[1].content.uuid,
            ).isEqualTo(skillWithCategory3.uuid)
        }

        @Test
        fun `search with keywords filter should apply to filtered search with AND operator between keywords`() {
            // Arrange
            val skillWithKeywords1and2 =
                TestObjectHelpers.randomRichSkillDoc().copy(
                    searchingKeywords = listOf("keyword1", "keyword2"),
                )
            val skillWithKeywords2and3 =
                TestObjectHelpers.randomRichSkillDoc().copy(
                    searchingKeywords = listOf("keyword2", "keyword3"),
                )
            val skillWithKeywords3and4 =
                TestObjectHelpers.randomRichSkillDoc().copy(
                    searchingKeywords = listOf("keyword3", "keyword4"),
                )

            richSkillEsRepo.saveAll(
                listOf(skillWithKeywords1and2, skillWithKeywords2and3, skillWithKeywords3and4),
            )

            // Act
            val filteredSearchResult1 =
                richSkillEsRepo.byApiSearch(
                    ApiSearch(
                        filtered =
                            ApiFilteredSearch(
            ...
            // Assert
            assertThat(
                filteredSearchResult1.searchHits
                    .first()
                    .content.uuid,
            ).isEqualTo(skillWithKeywords1and2.uuid)
            assertThat(
                filteredSearchResult2.searchHits
                    .first()
                    .content.uuid,
            ).isEqualTo(skillWithKeywords2and3.uuid)
            assertThat(
                filteredSearchResult3.searchHits
                    .first()
                    .content.uuid,
            ).isEqualTo(skillWithKeywords3and4.uuid)
        ...
        fun `search with standards filter should apply to filtered search with AND operator between standards`() {
            // Arrange
            val skillWithStandards1and2 =
                TestObjectHelpers.randomRichSkillDoc().copy(
                    standards = listOf("standard1", "standard2"),
                )
            val skillWithStandards2and3 =
                TestObjectHelpers.randomRichSkillDoc().copy(
                    standards = listOf("standard2", "standard3"),
                )
            val skillWithStandards3and4 =
                TestObjectHelpers.randomRichSkillDoc().copy(
                    standards = listOf("standard3", "standard4"),
                )

            richSkillEsRepo.saveAll(
                listOf(skillWithStandards1and2, skillWithStandards2and3, skillWithStandards3and4),
            )
            ...
            assertThat(
                filteredSearchResult1.searchHits
                    .first()
                    .content.uuid,
            ).isEqualTo(skillWithStandards1and2.uuid)
            assertThat(
                filteredSearchResult2.searchHits
                    .first()
                    .content.uuid,
            ).isEqualTo(skillWithStandards2and3.uuid)
            assertThat(
                filteredSearchResult3.searchHits
                    .first()
                    .content.uuid,
            ).isEqualTo(skillWithStandards3and4.uuid)
            assertThat(filteredSearchResult4).isEmpty()
        }
        ...
        fun `Search with certifications filter should apply to filtered search with AND operator between certifications`() {
            ...
            richSkillEsRepo.saveAll(
                listOf(
                    skillWithCertifications1and2,
                    skillWithCertifications2and3,
                    skillWithCertifications3and4,
                ),
            )
            ...
            assertThat(
                filteredSearchResult1.searchHits
                    .first()
                    .content.uuid,
            ).isEqualTo(skillWithCertifications1and2.uuid)
            assertThat(
                filteredSearchResult2.searchHits
                    .first()
                    .content.uuid,
            ).isEqualTo(skillWithCertifications2and3.uuid)
            assertThat(
                filteredSearchResult3.searchHits
                    .first()
                    .content.uuid,
            ).isEqualTo(skillWithCertifications3and4.uuid)
            assertThat(filteredSearchResult4).isEmpty()
        }
        ...
        fun `Search with alignments filter should apply to filtered search with AND operator between alignments`() {
            // Arrange
            val skillWithAlignments1and2 =
                TestObjectHelpers.randomRichSkillDoc().copy(
                    alignments = listOf("alignment1", "alignment2"),
                )
            val skillWithAlignments2and3 =
                TestObjectHelpers.randomRichSkillDoc().copy(
                    alignments = listOf("alignment2", "alignment3"),
                )
            val skillWithAlignments3and4 =
                TestObjectHelpers.randomRichSkillDoc().copy(
            ...
            richSkillEsRepo.saveAll(
                listOf(
                    skillWithAlignments1and2,
                    skillWithAlignments2and3,
                    skillWithAlignments3and4,
                ),
            )
            ...
            assertThat(
                filteredSearchResult1.searchHits
                    .first()
                    .content.uuid,
            ).isEqualTo(skillWithAlignments1and2.uuid)
            assertThat(
                filteredSearchResult2.searchHits
                    .first()
                    .content.uuid,
            ).isEqualTo(skillWithAlignments2and3.uuid)
            assertThat(
                filteredSearchResult3.searchHits
                    .first()
                    .content.uuid,
            ).isEqualTo(skillWithAlignments3and4.uuid)
            assertThat(filteredSearchResult4).isEmpty()
        }
        ...
        fun `Search with employers filter should apply to filtered search with AND operator between employers`() {
            // Arrange
            val skillWithEmployers1and2 =
                TestObjectHelpers.randomRichSkillDoc().copy(
                    employers = listOf("employer1", "employer2"),
                )
            val skillWithEmployers2and3 =
                TestObjectHelpers.randomRichSkillDoc().copy(
                    employers = listOf("employer2", "employer3"),
                )
            val skillWithEmployers3and4 =
                TestObjectHelpers.randomRichSkillDoc().copy(
                    employers = listOf("employer3", "employer4"),
                )

            richSkillEsRepo.saveAll(
                listOf(skillWithEmployers1and2, skillWithEmployers2and3, skillWithEmployers3and4),
            )
            ...
            assertThat(
                filteredSearchResult1.searchHits
                    .first()
                    .content.uuid,
            ).isEqualTo(skillWithEmployers1and2.uuid)
            assertThat(
                filteredSearchResult2.searchHits
                    .first()
                    .content.uuid,
            ).isEqualTo(skillWithEmployers2and3.uuid)
            assertThat(
                filteredSearchResult3.searchHits
                    .first()
                    .content.uuid,
            ).isEqualTo(skillWithEmployers3and4.uuid)
            assertThat(filteredSearchResult4).isEmpty()
        }
        ...
        fun `search with occupations filter should apply to filtered search`() {
            ...
            val skillWithOccupations1and2 =
                TestObjectHelpers
                    .randomRichSkillDoc()
                    .copy(jobCodes = jobCodes1and2)
            val skillWithOccupations2and3 =
                TestObjectHelpers
                    .randomRichSkillDoc()
                    .copy(jobCodes = jobCodes2and3)
            val skillWithOccupations3and4 =
                TestObjectHelpers
                    .randomRichSkillDoc()
            ...
            richSkillEsRepo.saveAll(
                listOf(
                    skillWithOccupations1and2,
                    skillWithOccupations2and3,
                    skillWithOccupations3and4,
                ),
            )
            ...
            assertThat(
                filteredSearchResult1.searchHits
                    .first()
                    .content.uuid,
            ).isEqualTo(skillWithOccupations1and2.uuid)
            assertThat(
                filteredSearchResult2.searchHits
                    .first()
                    .content.uuid,
            ).isEqualTo(skillWithOccupations2and3.uuid)
            assertThat(
                filteredSearchResult3.searchHits
                    .first()
                    .content.uuid,
            ).isEqualTo(skillWithOccupations3and4.uuid)
            assertThat(filteredSearchResult4).isEmpty()
        }
        ...
        fun `search with authors filter should apply to filtered search`() {
            // Arrange
            val skillWithAuthor1 =
                TestObjectHelpers.randomRichSkillDoc().copy(
                    authors = listOf("author1"),
                )
            val skillWithAuthor2 =
                TestObjectHelpers.randomRichSkillDoc().copy(
                    authors = listOf("author2"),
                )
            val skillWithAuthor3 =
                TestObjectHelpers.randomRichSkillDoc().copy(
            ...
            assertThat(
                filteredSearchResult.searchHits
                    .first()
                    .content.uuid,
            ).isEqualTo(skillWithAuthor1.uuid)
            assertThat(
                filteredSearchResult.searchHits[1].content.uuid,
            ).isEqualTo(skillWithAuthor3.uuid)
        }
        ...
        fun `search with authors & categories filter should apply to filtered search with AND operator between fields, and OR operator between values`() {
            // Arrange
            val skillWithAuthor1AndCategory1 =
                TestObjectHelpers.randomRichSkillDoc().copy(
                    authors = listOf("author1"),
                    categories = listOf("category1"),
                )
            val skillWithAuthor2AndCategory2 =
                TestObjectHelpers.randomRichSkillDoc().copy(
                    authors = listOf("author2"),
                    categories = listOf("category2"),
                )
            val skillWithAuthor1AndCategory3 =
                TestObjectHelpers.randomRichSkillDoc().copy(
                    authors = listOf("author1"),
                    categories = listOf("category3"),
                )
            val skillWithAuthor2AndCategory3 =
                TestObjectHelpers.randomRichSkillDoc().copy(
                    authors = listOf("author2"),
                    categories = listOf("category3"),
                )
            val skillWithAuthor3AndCategory3 =
                TestObjectHelpers.randomRichSkillDoc().copy(
                    authors = listOf("author3"),
                    categories = listOf("category3"),
                )
            ...
            richSkillEsRepo.saveAll(
                listOf(
                    skillWithAuthor1AndCategory1,
                    skillWithAuthor2AndCategory2,
                    skillWithAuthor3AndCategory3,
                    skillWithAuthor1AndCategory3,
                    skillWithAuthor2AndCategory3,
                ),
            )
            ...
            assertThat(
                filteredSearchResult1.searchHits
                    .first()
                    .content.uuid,
            ).isEqualTo(skillWithAuthor1AndCategory1.uuid)
            assertThat(
                filteredSearchResult1.searchHits[1].content.uuid,
            ).isEqualTo(skillWithAuthor2AndCategory2.uuid)
            ...
            assertThat(
                filteredSearchResult2.searchHits[4].content.uuid,
            ).isEqualTo(skillWithAuthor2AndCategory3.uuid)
        }
        ...
        fun `search with authors & categories & keywords filter should apply to filtered search with AND operator between fields, and OR operator between categories and authors and AND between keywords`() {
            ...
            assertThat(
                filteredSearchResult2.searchHits
                    .first()
                    .content.uuid,
            ).isEqualTo(skillWithAuthor3AndCategory3.uuid)
        }
        ...
        fun `search with categories, keywords & standards filter should apply to filtered search with AND operator between fields, and OR operator between categories and AND operator between keywords and standards`() {
            ...
            assertThat(
                filteredSearchResult1.searchHits
                    .first()
                    .content.uuid,
            ).isEqualTo(skill2.uuid)
            assertThat(filteredSearchResult2.searchHits).hasSize(1)
            assertThat(
                filteredSearchResult2.searchHits
                    .first()
                    .content.uuid,
            ).isEqualTo(skill3.uuid)
        ...
        fun testFindSimilar() {
            // Arrange
            val numOfSkills = 2L
            val skillByStatement1 =
                TestObjectHelpers.randomRichSkillDoc().copy(
                    statement = "Evaluate the level of listening to avoid listening traps.",
                )
            val skillByStatement2 =
                TestObjectHelpers.randomRichSkillDoc().copy(
                    statement = "Evaluate the level of listening to understand the message being conveyed.",
                )
            ...
            assertThat(
                result.searchHits.map { it.content.uuid }.toList(),
            ).contains(skillByStatement1.uuid)
        }
        ...
        fun `Should return an array of uuids without using query`() {
            ...
            val uuidsFromApiSearch =
                richSkillEsRepo.getUuidsFromApiSearch(
                    ApiSearch(uuids = uuids),
                    arrayOf(PublishStatus.Published).toSet(),
                    Pageable.unpaged(),
                    nullJwt,
                )
        ...
        fun `Should return an array of uuids using query`() {
            ...
            val uuids =
                richSkillEsRepo.getUuidsFromApiSearch(
                    ApiSearch(query = "export"),
                    arrayOf(PublishStatus.Draft, PublishStatus.Published).toSet(),
                    Pageable.unpaged(),
                    nullJwt,
                )
