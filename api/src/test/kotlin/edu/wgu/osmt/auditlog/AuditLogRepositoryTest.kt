...
class AuditLogRepositoryTest
    ...
    constructor(
        ...
        fun `can insert a collection audit log`() {
            val auditLog =
                AuditLog.Companion.fromAtomicOp(
                    CollectionTable,
                    1L,
                    listOf(),
                    "test user",
        ...
        fun `generates audit logs on collection update`() {
            ...
            val initialSkill =
                richSkillRepository.create(
                    RsdUpdateObject(name = "initial skill", statement = "test statement"),
                    testUser,
                )
            val addedSkill =
                richSkillRepository.create(
                    RsdUpdateObject(name = "added skill", statement = "test statement"),
                    testUser,
                )
            ...
            val collectionUpdateObject =
                CollectionUpdateObject(
                    ...
                    skills =
                        ListFieldUpdate(
                            add = listOf(addedSkill!!),
                            remove = listOf(initialSkill),
                        ),
            ...
            val logs =
                auditLogRepository
                    .findByTableAndId(CollectionTable.tableName, collectionDao!!.id.value)
                    .toList()
                    .map { it.toModel() }
            ...
            val publishStatusLog =
                logs.find {
                    it.operationType ==
                        AuditOperationType.PublishStatusChange.name
                }!!

            val initialRichSkillLogs =
                auditLogRepository
                    .findByTableAndId(RichSkillDescriptorTable.tableName, initialSkill.id.value)
                    .toList()
                    .map { it.toModel() }
            val updatedRichSkillLogs =
                auditLogRepository
                    .findByTableAndId(RichSkillDescriptorTable.tableName, addedSkill.id.value)
                    .toList()
                    .map { it.toModel() }

            assertThat(
                initialLog.changedFields.findByFieldName(CollectionDao::name.name),
            ).isEqualTo(
                Change(
            ...
            // updates
            assertThat(
                updateLog.changedFields.findByFieldName(CollectionDao::author.name),
            ).isEqualTo(
                Change(
            ...
            assertThat(
                publishStatusLog.changedFields.findByFieldName(CollectionDao::publishStatus.name),
            ).isEqualTo(
                Change(
            ...
            assertThat(
                initialRichSkillLogs
                    .find {
                        it.operationType == AuditOperationType.Update.name
                    }!!
                    .changedFields
                    .findByFieldName(
                ...
                Change(
                    RichSkillDescriptorDao::collections.name,
                    old = initialCollectionUpdate.name,
                    new = null,
                ),
            )

            assertThat(
                updatedRichSkillLogs
                    .find {
                        it.operationType == AuditOperationType.Update.name
                    }!!
                    .changedFields
        ...
        fun `generates audit logs on rich skill update`() {
            ...
            val skill =
                richSkillRepository.create(
                    RsdUpdateObject(name = initialSkillName, statement = initialStatement),
                    testUser,
                )
            val authorDaos = listOf(keywordRepository.getDefaultAuthor())
            val categoryDaos =
                TestObjectHelpers
                    .keywordsGenerator(2, KeywordTypeEnum.Category)
                    .mapNotNull { keywordRepository.create(it.type, it.value) }
            val keywordDaos =
                TestObjectHelpers
                    .keywordsGenerator(10, KeywordTypeEnum.Keyword)
                    .mapNotNull { keywordRepository.create(it.type, it.value) }
            val jobCodeDaos = listOf(jobCodeRepository.create("11-1170"))
            val collectionDaos =
                listOf(collectionRepository.create("test collection", testUser, testEmail)!!)
            ...
            val updateLogs =
                auditLogRepository
                    .findByTableAndId(RichSkillDescriptorTable.tableName, skill.id.value)
                    .map { it.toModel() }
                    .filter { it.operationType == AuditOperationType.Update.name }
                    .sortedBy { it.creationDate }

            val publishStatusLogs =
                auditLogRepository
                    .findByTableAndId(RichSkillDescriptorTable.tableName, skill.id.value)
                    .map { it.toModel() }
                    .filter { it.operationType == AuditOperationType.PublishStatusChange.name }
                    .sortedBy { it.creationDate }
            ...
            assertThat(
                firstUpdateLog.changedFields.findByFieldName(RichSkillDescriptor::name.name),
            ).isEqualTo(
                Change(
            ...
            assertThat(
                firstUpdateLog.changedFields.findByFieldName(RichSkillDescriptor::statement.name),
            ).isEqualTo(
                Change(
            ...
            assertThat(
                firstUpdateLog.changedFields.findByFieldName(RichSkillDescriptor::authors.name),
            ).isEqualTo(
                Change(
            ...
            assertThat(
                firstUpdateLog.changedFields.findByFieldName(RichSkillDescriptor::categories.name),
            ).isEqualTo(
                Change(
            ...
            assertThat(
                firstUpdateLog.changedFields.findByFieldName(
            ...
            assertThat(
                firstUpdateLog.changedFields.findByFieldName(RichSkillDescriptor::collections.name),
            ).isEqualTo(
                Change(
            ...
            assertThat(
                firstUpdateLog.changedFields.findByFieldName(RichSkillDescriptor::jobCodes.name),
            ).isEqualTo(
                Change(
            ...
            assertThat(
                secondUpdateLog.changedFields.findByFieldName(RichSkillDescriptor::authors.name),
            ).isEqualTo(
                Change(
                    RichSkillDescriptor::authors.name,
            ...
            assertThat(
                secondUpdateLog.changedFields.findByFieldName(RichSkillDescriptor::categories.name),
            ).isEqualTo(
                Change(
                    RichSkillDescriptor::categories.name,
            ...
            assertThat(
                secondUpdateLog.changedFields.findByFieldName(
                    RichSkillDescriptor::searchingKeywords.name,
                ),
            ).isEqualTo(
                Change(
            ...
            assertThat(
                secondUpdateLog.changedFields.findByFieldName(RichSkillDescriptor::jobCodes.name),
            ).isEqualTo(
                Change(
                    RichSkillDescriptor::jobCodes.name,
            ...
            assertThat(
                publishStatusLogs
                    ...
                    .findByFieldName(RichSkillDescriptor::publishStatus.name),
            ).isEqualTo(
                Change(
        ...
        fun `generates rich skill audit log for collection name changes`() {
            ...
            val collectionUpdate =
                CollectionUpdateObject(id = collectionDao?.id?.value, name = updatedCollectionName)
            collectionRepository.update(collectionUpdate, testUser)

            val auditLogResults =
                collectionDao?.id?.let {
                    auditLogRepository
                        .findByTableAndId(CollectionTable.tableName, it.value)
                        .find { it.operationType == AuditOperationType.Update.name }
                        ?.toModel()
                }
